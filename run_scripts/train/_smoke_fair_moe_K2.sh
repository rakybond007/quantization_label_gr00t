#!/bin/bash
# Smoke fair_moe K=2 {raw16(main), merged8(m8)} ONLY (no m4/n8), balance off.
# 5 steps, 2 GPU. Validates the K=2 code path (asserts/horizons/targets/decoders).
set -u
DATA_CFG=${1:-single_panda_gripper_multi_horizon}

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
OUT_DIR="$BASE_DIR/ckpt/_smoke_fair_moe_K2_robocasa"
...
echo "[$(date '+%T')] === SMOKE fair_moe K=2 {raw16, merged8}, balance off, data=$DATA_CFG ==="
...
"$CONDA_PATH/envs/gr00t/bin/python" $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name SMOKE-fair-moe-K2-raw16-merged8 \
