#!/bin/bash
# Smoke per_quad_mask + n8 (MoE4 extension): 4 quads × {4 or 3} options.
# quads 0,1: main 4-raw / m8 2-comp / m4 1-deep / n8 4-raw
# quads 2,3: main 4-raw / m8 2-comp / m4 1-deep (n8 masked)
# 5 steps, 2 GPU, wandb.
# Usage: bash _smoke_moe_per_quad_mask_n8.sh [robocasa|libero]
set -u
...
OUT_DIR="$BASE_DIR/ckpt/_smoke_moe_per_quad_mask_n8_${TARGET}"
# Strip env vars that route imports to a sibling workspace (e.g. a tmux pane
# left in a different VLA repo with PYTHONPATH=<that repo>).
unset VIRTUAL_ENV PYTHONHOME PYTHONPATH
export PATH="$CONDA_PATH/envs/gr00t/bin:$PATH"
PYBIN="$CONDA_PATH/envs/gr00t/bin/python"
# Force CWD so the workspace `gr00t` package is imported (not a sibling
# workspace's `gr00t` that might be on Python's default search path).
cd "$BASE_DIR"

echo "[$(date '+%T')] === SMOKE: $TARGET per_quad_mask+n8 (5 steps) ==="
...
"$PYBIN" $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name SMOKE-$TARGET-moe-per-quad-mask-n8 \
    --batch-size 2 --num-gpus 1 --max-steps 5 --save-steps 100000 \
    --report-to wandb \
    $DDIMS \
