#!/bin/bash
#SBATCH --job-name=groot_n1_5_libero_moe4_v1_K4_b_only_no_metaq_router_instr_end_60k_bs64
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="Libero MoE4 v1 (u16/c8/c4/n8) + [B] EMA-norm KL only + no metaq. Router input = instruction_end (last vl token) replacing avg_pool. fair_moe fork."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-train_libero_moe4_v1_K4_b_only_no_metaq_router_instr_end.out
#SBATCH --error=out/%j-train_libero_moe4_v1_K4_b_only_no_metaq_router_instr_end.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-libero
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/libero/groot/groot_n1_5_bs64_fair_moe_v1_K4_b_only_no_metaq_router_instr_end"
...
python $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name GR00T-N1.5-libero-moe4-v1-K4-b-only-no-metaq-router-instr-end-bs64 \
