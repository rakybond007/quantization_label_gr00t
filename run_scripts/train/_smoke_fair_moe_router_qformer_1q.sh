#!/bin/bash
# Smoke fair_moe fork + router_input_type=qformer_1q.
# 5 steps, 2 GPU.
set -u
...
OUT_DIR="$BASE_DIR/ckpt/_smoke_fair_moe_router_qformer_1q_robocasa"
...
"$CONDA_PATH/envs/gr00t/bin/python" $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name SMOKE-fair-moe-router-qformer-1q \
    ...
    --moe-router-input-type=qformer_1q --moe-router-qformer-heads=4 \
    --save-total-limit 2
