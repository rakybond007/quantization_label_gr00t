#!/bin/bash
#SBATCH --job-name=groot_n1_5_gr1_tabletop_mh_m8_econsist
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="GR00T N1.5 finetune on RoboCasa GR-1 Tabletop: multi-horizon + merged_8 + ensemble-consistency"
#SBATCH --partition=batch
#SBATCH --output=out/%j-groot_n1_5_gr1_tabletop_mh_m8_econsist.out
#SBATCH --error=out/%j-groot_n1_5_gr1_tabletop_mh_m8_econsist.err

# 2-GPU training matching robocasa mh_m8_econsist setup, applied to GR-1 tabletop.
# Loss: 1.0*L_main16 + 1.0*L_m8 + warmup*(1.0*L_f2 + 1.0*L_f4) + c_warmup*0.1*L_consist
# Global batch = 2 * 32 = 64.
#
# Action layout (29 dims, concat order = arm,arm,hand,hand,waist):
#   left_arm 0-6, right_arm 7-13, left_hand 14-19, right_hand 20-25, waist 26-28.
# All dims are continuous (Fourier hands are joint positions, not binary
# grippers), so --discrete-action-dims is OMITTED for GR-1.
...
CKPT_DIR="$BASE_DIR/ckpt/gr1_tabletop/groot/groot_n1_5_bs64_mh_m8_econsist"
...
python $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --data-config fourier_gr1_arms_waist_multi_horizon \
    --embodiment-tag gr1 \
    --base-model-path nvidia/GR00T-N1.5-3B \
    --run-name GR00T-N1.5-gr1tabletop-mh-m8-econsist-fromPT-bs64 \
    --batch-size 32 \
    --num-gpus 2 \
    --max-steps 60000 \
    --save-steps 10000 \
