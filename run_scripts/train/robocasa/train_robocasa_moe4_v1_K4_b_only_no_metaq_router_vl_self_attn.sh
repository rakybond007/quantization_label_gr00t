#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_moe4_v1_K4_b_only_no_metaq_router_vl_self_attn_60k_bs64
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="Robocasa MoE4 v1 (u16/c8/c4/n8) + [B] EMA-norm KL only + no metaq. Router input = router-private vlln + SelfAttentionTransformer (action-head-style vl ingestion) + mean-pool."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-train_robocasa_moe4_v1_K4_b_only_no_metaq_router_vl_self_attn.out
#SBATCH --error=out/%j-train_robocasa_moe4_v1_K4_b_only_no_metaq_router_vl_self_attn.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_fair_moe_v1_K4_b_only_no_metaq_router_vl_self_attn"
...
python $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name GR00T-N1.5-robocasa-moe4-v1-K4-b-only-no-metaq-router-vl-self-attn-bs64 \
    ...
    --moe-router-input-type=vl_self_attn \
    --no-moe-shared-t \
