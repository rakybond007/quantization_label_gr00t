#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_fair_moe_v2_u16_c8_u24_c12_b_only_no_a_no_c_no_d_bs64
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="V2 decoders u16/c8/u24/c12 (16- and 24-coverage) + fair_moe fork (no metaq) + [B] EMA-norm KL only (no [A] shared_t, no [C] switch balance, no [D] length cost)."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-groot_n1_5_robocasa_fair_moe_v2_b_only.out
#SBATCH --error=out/%j-groot_n1_5_robocasa_fair_moe_v2_b_only.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_fair_moe_v2_b_only"

source $CONDA_PATH/bin/activate gr00t
python $BASE_DIR/scripts/gr00t_finetune_fair_moe_v2.py \
    ...
    --run-name GR00T-N1.5-robocasa-fair-moe-v2-b-only-bs64 \
