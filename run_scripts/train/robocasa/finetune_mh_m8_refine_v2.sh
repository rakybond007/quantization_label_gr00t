#!/bin/bash
#SBATCH --job-name=mh_m8_refine_v2
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="[Method 2] Fine-tune mh_m8 from 60k ckpt + add post-hoc m8 refinement MLP"
#SBATCH --partition=batch
#SBATCH --output=out/%j-mh_m8_refine_v2.out
#SBATCH --error=out/%j-mh_m8_refine_v2.err
...
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_econsist_discfix_refine_v2"
...
python $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name GR00T-N1.5-mh_m8_econsist-refine-v2 \
    --batch-size 32 \
    --num-gpus 2 \
    --max-steps 30000 \
    --save-steps 10000 \
