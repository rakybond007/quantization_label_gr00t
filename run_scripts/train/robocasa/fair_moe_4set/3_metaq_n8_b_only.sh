#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_metaq_n8_b_only_no_a_no_c_no_d_bs64_60k
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="metaq N=8 + [B] EMA-norm KL only (no [A] shared_t, no [C] switch balance, no [D] length cost).  Pair with fair_moe variant 1/2 for ablation."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-groot_n1_5_robocasa_metaq_n8_b_only.out
#SBATCH --error=out/%j-groot_n1_5_robocasa_metaq_n8_b_only.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_metaq_n8_b_only"
...
python $BASE_DIR/scripts/gr00t_finetune_metaq.py \
    ...
    --run-name GR00T-N1.5-robocasa-metaq-n8-b-only-bs64 \
