#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_fair_moe_b_only_state_aug_bs64_60k
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="fair_moe fork (no metaq): [B] EMA-norm KL only.  + StateActionPerturbation/Dropout data aug."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-groot_n1_5_robocasa_fair_moe_b_only_state_aug.out
#SBATCH --error=out/%j-groot_n1_5_robocasa_fair_moe_b_only_state_aug.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_fair_moe_b_only_state_aug"
...
python $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --data-config single_panda_gripper_multi_horizon_state_aug \
    --embodiment-tag new_embodiment \
    --base-model-path nvidia/GR00T-N1.5-3B \
    --run-name GR00T-N1.5-robocasa-fair-moe-b-only-state-aug-bs64 \
