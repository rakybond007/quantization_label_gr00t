#!/bin/bash
#SBATCH --job-name=mh_m8_distill_from_f2
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="[Method 1] Fine-tune mh_m8 from 60k ckpt with strong f2->m8 distillation"
#SBATCH --partition=batch
#SBATCH --output=out/%j-mh_m8_distill.out
#SBATCH --error=out/%j-mh_m8_distill.err
...
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_econsist_discfix_distill"
...
python $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name GR00T-N1.5-mh_m8_econsist-distill-from-f2 \
    --batch-size 32 \
    --num-gpus 2 \
    --max-steps 10000 \
    --save-steps 2000 \
