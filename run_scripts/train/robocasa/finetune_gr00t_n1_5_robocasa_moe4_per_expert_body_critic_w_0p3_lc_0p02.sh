#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_moe4_per_expert_body_critic_w_0p3_lc_0p02
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="GR00T N1.5 robocasa: MoE4 (main+m8+m4+n8) + compression-safety critic (w=0.3) + length cost (gamma=0.02)"
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-groot_n1_5_robocasa_moe4_critic.out
#SBATCH --error=out/%j-groot_n1_5_robocasa_moe4_critic.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_moe4_per_expert_body_critic_w_0p3_lc_0p02"
...
python $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name GR00T-N1.5-robocasa-moe4-critic-w-0p3-lc-0p02-bs64 \
