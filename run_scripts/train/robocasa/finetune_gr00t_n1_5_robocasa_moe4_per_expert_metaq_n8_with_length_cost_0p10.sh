#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_moe4_per_expert_metaq_n8_shared_t_ema_norm_switch_balance_length_cost_0p10
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="MoE4 per_expert single_pick + meta-query N=8 + [A] shared_t + [B] EMA-norm KL + [C] switch balance + [D] length cost γ=0.10 in EMA-normalized loss units."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-groot_n1_5_robocasa_metaq_with_d.out
#SBATCH --error=out/%j-groot_n1_5_robocasa_metaq_with_d.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_moe4_per_expert_metaq_n8_length_cost_0p10"
...
python $BASE_DIR/scripts/gr00t_finetune_metaq.py \
    ...
    --run-name GR00T-N1.5-robocasa-moe4-per-expert-metaq-n8-length-cost-0p10-bs64 \
