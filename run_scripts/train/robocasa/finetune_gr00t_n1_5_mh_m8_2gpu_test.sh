#!/bin/bash
# 2-GPU short test of multi-horizon + discrete-fix + merged_8 (native 8-step) head.
# Verifies model loads, dataloader works, training step succeeds, checkpoint
# saves, and that loss_main / loss_m8 / loss_f2 / loss_f4 all appear in logs.
# Use: bash run_scripts/train/robocasa/finetune_gr00t_n1_5_mh_m8_2gpu_test.sh
...
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_mh_m8_2gpu_test"
...
python $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name GR00T-N1.5-mh-m8-2gpu-test \
    --batch-size 4 \
    --num-gpus 2 \
    --max-steps 20 \
    --save-steps 10 \
