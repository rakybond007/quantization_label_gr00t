#!/bin/bash
#SBATCH --job-name=groot_n1_5_robocasa_moe4_v1_K4_b_only_no_metaq_60k_bs64_fair_moe_fork
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="Robocasa MoE4 v1 (u16/c8/c4/n8) + [B] EMA-norm KL only + no metaq + no [D]. fair_moe fork (no meta-query tokens)."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-train_robocasa_moe4_v1_K4_b_only_no_metaq.out
#SBATCH --error=out/%j-train_robocasa_moe4_v1_K4_b_only_no_metaq.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-robocasa
mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_fair_moe_v1_K4_b_only_no_metaq"
...
python $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name GR00T-N1.5-robocasa-moe4-v1-K4-b-only-no-metaq-bs64 \
