#!/bin/bash
# Smoke per-quad-mask MoE training (5 steps, 2 GPU, wandb).
# Usage: bash _smoke_moe_per_quad_mask.sh [robocasa|libero]
set -u
...
OUT_DIR="$BASE_DIR/ckpt/_smoke_moe_per_quad_mask_${TARGET}"
# Strip any active venv (e.g. alinvla uv venv on the worker node) so the
# nested `torchrun` call inside gr00t_finetune.py picks the conda gr00t
# binaries instead of the venv ones.
unset VIRTUAL_ENV PYTHONHOME
export PATH="$CONDA_PATH/envs/gr00t/bin:$PATH"
PYBIN="$CONDA_PATH/envs/gr00t/bin/python"

echo "[$(date '+%T')] === SMOKE: $TARGET MoE per_quad_mask (5 steps) ==="
...
"$PYBIN" $BASE_DIR/scripts/gr00t_finetune.py \
    ...
    --run-name SMOKE-$TARGET-moe-per-quad-mask \
