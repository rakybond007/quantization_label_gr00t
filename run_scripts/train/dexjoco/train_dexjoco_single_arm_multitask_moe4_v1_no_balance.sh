#!/bin/bash
#SBATCH --job-name=groot_n1_5_dexjoco_single_arm_multitask_6tasks_moe4_v1_no_balance_60k_bs64_2gpu
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --comment="DexJoCo single-arm multi-task (6 tasks via LeRobotMixtureDataset) MoE4 v1 (balance off). Absolute actions -> action-mode abs."
#SBATCH --partition=sjw_alinlab
#SBATCH --output=out/%j-train_dexjoco_single_arm_multitask_moe.out
#SBATCH --error=out/%j-train_dexjoco_single_arm_multitask_moe.err

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"; CONDA_PATH="$HOME/miniconda3"
export WANDB_PROJECT=GR00T-dexjoco; mkdir -p out
CKPT_DIR="$BASE_DIR/ckpt/dexjoco/groot/groot_n1_5_bs64_single_arm_multitask_moe4_v1_no_balance"
...
python $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name GR00T-N1.5-dexjoco-single-arm-multitask-moe4-v1-no-balance-bs64 \
