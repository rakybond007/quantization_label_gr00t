#!/bin/bash
# Smoke fair_moe fork + router_input_type=instruction_end.
# 5 steps, 2 GPU.
set -u
...
OUT_DIR="$BASE_DIR/ckpt/_smoke_fair_moe_router_instr_end_robocasa"
...
"$CONDA_PATH/envs/gr00t/bin/python" $BASE_DIR/scripts/gr00t_finetune_fair_moe.py \
    ...
    --run-name SMOKE-fair-moe-router-instr-end \
