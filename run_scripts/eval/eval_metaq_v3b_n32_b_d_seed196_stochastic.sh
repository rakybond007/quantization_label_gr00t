#!/bin/bash
#SBATCH --job-name=eval_robocasa_metaq_v3b_n32_K6_B_D_lc_0p10_HF_60k_seed196_STOCHASTIC_24t_50ep_arr8_seed_variance_probe
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_metaq_v3b_n32_b_d_seed196_stochastic.out
#SBATCH --error=out/%A_%a-eval_metaq_v3b_n32_b_d_seed196_stochastic.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="metaq v3b (K=6) + B + D gamma=0.10 + n_meta_q=32 (HF, best so far) @ 60k — seed 196 (seed-variance probe). STOCHASTIC router pick (sample by probs)."

set -u

PORT=$((10130 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/metaq_v3b_n32_b_d_lc_0p10_stochastic_seed196"
