#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_moe4_v1_no_balance_60k_force_uniform_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_force_uniform.out
#SBATCH --error=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_force_uniform.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo 6-task MoE4 v1 (no balance) FORCE_UNIFORM router pick (head=moe + --moe-force-uniform) @60k, 50 ep/task. Sanity: tests if router decision is meaningful vs. random expert choice."
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/single_arm_multitask_moe4_v1_no_balance_60k_force_uniform"
...
"$CONDA_PATH/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/serve_policy_dexjoco.py" \
    ...
    --head moe --denoising-steps 4 --moe-force-uniform \
    > "$SERVER_LOG" 2>&1 &
SPID=$!
READY=0
