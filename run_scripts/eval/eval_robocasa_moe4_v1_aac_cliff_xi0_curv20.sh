#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe4_v1_K4_b_only_no_metaq_HF_60k_aac_cliff_xi0_curv20_m8_moe_selective_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_moe4_v1_aac_cliff_xi0_curv20.out
#SBATCH --error=out/%A_%a-eval_robocasa_moe4_v1_aac_cliff_xi0_curv20.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa MoE4 v1 (K=4, fair_moe) + AAC cliff (head=moe_selective N=10, entropy, xi=0 + curvature 2.0, compress_src=m8 trained decoder). Matches baseline+aac best (xi0+curv2.0) for breakwater comparison."

set -u
HF_REPO=prehj/GR00T-N1.5-robocasa-moe4-v1-K4-b-only-no-metaq-60k
PORT=$((9850 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
N_SAMPLES=10
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe4_v1_b_only_no_metaq_aac_cliff_xi0_curv20"
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 "$CONDA_PATH/envs/robocasa_gr00t/bin/python" -u \
        ...
        --aac-xi 0 --aac-compress-src m8 --curvature-weight 2.0 \
