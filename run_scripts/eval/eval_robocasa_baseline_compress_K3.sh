#!/bin/bash
#SBATCH --job-name=eval_robocasa_baseline_compress_K3_quantization_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_baseline_compress_K3.out
#SBATCH --error=out/%A_%a-eval_robocasa_baseline_compress_K3.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa GR00T-N1.5 baseline @60k + fixed-K=3 action quantization (continuous=sum, discrete=last). 24 tasks x 50 ep."

set -u
CKPT_DIR="$HOME/multigpu_workspace/Isaac-GR00T/ckpt/robocasa/groot/groot_n1_5_bs64_baseline/checkpoint-60000"
PORT=$((10160 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
K=3
