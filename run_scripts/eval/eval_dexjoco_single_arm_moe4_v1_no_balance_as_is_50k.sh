#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_6tasks_moe4_v1_no_balance_50k_as_is_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_as_is.out
#SBATCH --error=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_as_is.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo single-arm 6-task MoE4 v1 (no balance) router pick (head=moe) @50k, 50 ep/task."
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/single_arm_multitask_moe4_v1_no_balance_50k_as_is"
