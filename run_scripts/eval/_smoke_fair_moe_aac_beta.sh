#!/bin/bash
# Smoke: fair_moe + state_aug @ 60k, head=moe_selective, N=10 samples,
# Beta-sampled timestep schedule at inference, AAC cliff (entropy score).
# 1 task × 2 ep on CloseDrawer; verifies the new --beta_sample_t flag end-to-end.
set -u
...
echo "[$(date '+%T')] === SMOKE fair_moe + AAC cliff + Beta sampling: starting server ==="
