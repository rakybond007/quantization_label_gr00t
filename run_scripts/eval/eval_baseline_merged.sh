#!/bin/bash
#SBATCH --job-name=eval_baseline_merged
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="Baseline GR00T-N1.5 (no multi-horizon) + client-side 16->8 action merging"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_baseline_merged.out
#SBATCH --error=out/%A_%a-eval_baseline_merged.err
#SBATCH --time=2-00:00:00

# Pilot-style eval: baseline (vanilla 16-horizon) checkpoint, but client merges
# adjacent action pairs into 8-step chunks (continuous=sum, discrete=later).
# Tests whether client-side compression on a vanilla model matches/beats our
# multi-horizon trained model.

set -u

PORT=$((8890 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_baseline/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/baseline_merged_full"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | baseline-merged | port=$PORT"
