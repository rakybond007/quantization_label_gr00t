#!/bin/bash
#SBATCH --job-name=eval_libero_mh_m8_econsist_ensemble_fix
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_mh_m8_econsist_ensemble_fix.out
#SBATCH --error=out/%A_%a-eval_libero_mh_m8_econsist_ensemble_fix.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="Eval our libero mh_m8_econsist ensemble_fix (60k) on 4 task suites x 10 tasks x 50 ep"
...
OUTPUT_BASE="$BASE_DIR/output/libero/mh_m8_econsist_ensemble_fix"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | libero mh_m8_econsist ensemble_fix | port=$PORT"
