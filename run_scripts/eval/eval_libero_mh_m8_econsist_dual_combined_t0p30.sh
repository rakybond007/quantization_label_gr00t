#!/bin/bash
#SBATCH --job-name=eval_libero_dual_combined_t0p30
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_dual_combined_t0p30.out
#SBATCH --error=out/%A_%a-eval_libero_dual_combined_t0p30.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="Eval our libero dual combined thresh=0.30 (60k) on 4 task suites x 10 tasks x 50 ep"
...
OUTPUT_BASE="$BASE_DIR/output/libero/mh_m8_econsist_dual_combined_t0p30"
...
PORT=$((8410 + SLURM_ARRAY_TASK_ID))
echo "[i] Array $SLURM_ARRAY_TASK_ID | libero dual combined thresh=0.30 | port=$PORT"
...
for TASK_NAME in "${TASK_NAMES[@]}"; do
    ...
    mkdir -p "$OUTPUT_DIR"
    "$CONDA_PATH/envs/libero/bin/python" "$BASE_DIR/gr00t/eval/libero/eval_taskwise_gr00t_dual.py" \
        --args.task-suite-name $TASK_NAME \
        --args.video-out-path $OUTPUT_DIR \
        --args.task_idx=$SLURM_ARRAY_TASK_ID \
        --args.port=$PORT \
        --args.decision=combined \
        --args.agreement_thresh=0.30 \
        --args.replan_steps=5 \
        --args.replan_steps_m8=3 \
