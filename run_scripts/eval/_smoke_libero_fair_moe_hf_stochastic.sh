#!/bin/bash
# Smoke libero fair_moe HF ckpt + --moe-stochastic.
# Usage: bash _smoke_libero_fair_moe_hf_stochastic.sh <HF_REPO> <TAG> [PORT]
set -u
...
"$CONDA_PATH/envs/gr00t/bin/python" "$BASE_DIR/scripts/serve_policy_fair_moe.py" \
    ...
    --discrete-action-dims 6 --moe-stochastic \
    > "$OUT/server.log" 2>&1 &
SPID=$!
...
sleep 20
grep -E "MoE stochastic|inference head" "$OUT/server.log"
...
if [ $RC -eq 0 ]; then echo "[$TAG SMOKE_PASS]"; else echo "[$TAG SMOKE_FAIL]"; tail -20 "$OUT/eval.log"; fi
exit $RC
