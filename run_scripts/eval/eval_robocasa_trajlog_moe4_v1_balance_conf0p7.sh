#!/bin/bash
#SBATCH --job-name=eval_robocasa_trajlog_moe4_v1_balance_conf0p7_HF_60k_24t_3ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_trajlog_moe.out
#SBATCH --error=out/%A_%a-eval_robocasa_trajlog_moe.err
#SBATCH --time=1-00:00:00
#SBATCH --comment="robocasa MoE4 v1 b_only no_metaq HF-60k balance + conf0p7 + trajectory logging. Same 24 tasks x 3 ep, same seed as baseline trajlog for trajectory comparison."

set -u
HF_REPO=prehj/GR00T-N1.5-robocasa-moe4-v1-K4-b-only-no-metaq-60k
PORT=$((10600 + SLURM_ARRAY_TASK_ID))
N_EPISODES=15
SEED=42
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/_trajlog/moe4_v1_balance_60k_conf0p7"
