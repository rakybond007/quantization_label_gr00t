#!/bin/bash
#SBATCH --job-name=eval_robocasa_baseline_aac_cliff_entropy_xi0_main_n_main_sumpair_60k_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --comment="Robocasa BASELINE (non-MoE) + AAC cliff (entropy, xi=0 (no floor, allows 0 compression)) via head=main_n + main_sumpair. Inference-time sum-pair compression on the base model (no m8/MoE). 24 tasks x 50 ep."
...
PORT=$((8650 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_baseline/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/baseline_aac_cliff_xi${AAC_XI}"
