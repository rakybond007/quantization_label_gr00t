#!/bin/bash
#SBATCH --job-name=eval_libero_moe4_v1_K4_ema0p95_HF_60k_conf0p7_gated_stochastic_trial2_4suite_10arr
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --comment="libero MoE4 v1 (K=4 u16/c8/c4/n8) + B only + warmup=10k EMA=0.95 bs64 HF @60k, STOCHASTIC router pick, all 4 suites."

set -u
HF_REPO=prehj/GR00T-N1.5-libero-moe4-v1-K4-b-only-no-metaq-warmup10k-ema0p95-bs64-60k
PORT=$((9360 + SLURM_ARRAY_TASK_ID))
...
OUTPUT_BASE="$BASE_DIR/output/libero/moe4_v1_K4_warmup10k_ema0p95_bs64_hf_conf0p7_trial2"
...
"$CONDA_PATH/envs/gr00t/bin/python" "$BASE_DIR/scripts/serve_policy_fair_moe.py" \
    --port=$PORT --model-path=$CKPT --head=moe --n-samples=1 --moe-stochastic --moe-confidence-threshold 0.7 \
...
for TASK_NAME in "${TASK_NAMES[@]}"; do
    ...
    mkdir -p "$OUTPUT_DIR"
    OPENPI_SRC="$HOME/multigpu_workspace/openpi/packages/openpi-client/src" PYTHONPATH="$OPENPI_SRC" "$CONDA_PATH/envs/libero/bin/python" "$BASE_DIR/gr00t/eval/libero/eval_taskwise_gr00t_moe.py" \
