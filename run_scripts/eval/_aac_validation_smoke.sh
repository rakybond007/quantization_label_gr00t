#!/bin/bash
# Validate that aac_cliff + aac_chunk_binary actually run end-to-end with
# step-level AAC on entropy score. 1 task × 3 ep each.
set -u
...
OUT=$BASE/output/robocasa/_smoke_selective/_aac_val
...
run_one() {
    ...
    echo "[$(date '+%T')]     -> succ=$succ/$ep  $meanlen  errors=$err"
}

run_one aac_cliff_xi1   "--decision-rule aac_cliff --aac-xi 1"
run_one aac_binary_K4   "--decision-rule aac_chunk_binary --aac-xi 4"
