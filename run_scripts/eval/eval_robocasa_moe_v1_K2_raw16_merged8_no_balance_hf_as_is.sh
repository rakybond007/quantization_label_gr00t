#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe_v1_K2_raw16_merged8_b_only_no_metaq_no_balance_HF_60k_as_is_router_pick_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --comment="robocasa MoE K=2 {raw16, merged8} + B only + no metaq + no_balance HF ckpt @60k, as_is router pick."

set -u
HF_REPO=prehj/GR00T-N1.5-robocasa-moe-v1-K2-raw16-merged8-b-only-no-metaq-no-balance-60k
PORT=$((9740 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe_v1_K2_raw16_merged8_b_only_no_metaq_no_balance_hf_as_is"
