#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_6tasks_pi0_fast_baseline_20k_padded_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_pi0_fast_baseline_20k_padded.out
#SBATCH --error=out/%A_%a-eval_dexjoco_pi0_fast_baseline_20k_padded.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo single-arm 6-task pi0-fast baseline @20k WITH PATCHED FAST DECODER (zero-pad partial chunks instead of all-zero fallback). 50 ep/task."

set -u
TASKS=(hammer_nail click_mouse pick_bucket pinch_tongs fold_glasses water_plant)
TASK="${TASKS[$SLURM_ARRAY_TASK_ID]}"
PORT=$((9720 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
CKPT="$OPENPI_DIR/checkpoints/pi0_fast_dexjoco_single_arm_baseline/pi0_fast_dexjoco_single_arm_baseline_h100_2gpu_perGPU32_bs64_60k/20000"
CONFIG="$DEXJOCO_REPO/configs/rand_obj/$TASK.yaml"
OUTPUT_BASE="$GR00T_BASE/output/dexjoco/single_arm_multitask_pi0_fast_baseline_20k_padded"
...
echo "[i] $TASK arr=$SLURM_ARRAY_TASK_ID port=$PORT (PATCHED FAST decoder)"
cd "$OPENPI_DIR"
"$OPENPI_VENV/bin/python" -u _patched_serve_policy.py \
    --port $PORT \
    policy:checkpoint \
    --policy.config=pi0_fast_dexjoco_single_arm_baseline \
