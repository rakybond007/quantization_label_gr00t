#!/bin/bash
#SBATCH --job-name=eval_robocasa_fair_moe_b_only_state_aug_60k_chkpt_as_is_router_pick_24t_50ep_arr8_local
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_fair_moe_b_only_state_aug_as_is.out
#SBATCH --error=out/%A_%a-eval_robocasa_fair_moe_b_only_state_aug_as_is.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="fair_moe + [B] EMA-norm KL only + state_aug DataConfig (local 318764) eval at 60k, as_is router pick."

set -u
PORT=$((9770 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_fair_moe_b_only_state_aug/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/fair_moe_b_only_state_aug_as_is"
