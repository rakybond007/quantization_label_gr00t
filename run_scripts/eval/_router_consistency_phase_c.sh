#!/bin/bash
# Phase C: forced compression schedule (B: from prior, C: uniform random)
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/_router_consistency/phase_c"
...
"$CONDA_PATH/envs/dexjoco/bin/python" -u "$BASE_DIR/scripts/dexjoco_router_consistency.py" \
    ...
    --output-jsonl "$OUTPUT_BASE/rollouts_C.jsonl" \
    --mode phase_c \
    --prior-jsonl "$PRIOR" \
