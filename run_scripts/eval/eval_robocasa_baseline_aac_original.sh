#!/bin/bash
#SBATCH --job-name=eval_robocasa_baseline_aac_original_paper_dynamic_horizon_N10_xi1_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --exclude=worker-node100
#SBATCH --output=out/%A_%a-eval_robocasa_baseline_aac_original.out
#SBATCH --error=out/%A_%a-eval_robocasa_baseline_aac_original.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa GR00T-N1.5 baseline @60k + original AAC paper dynamic-horizon (head=main_n, N=10 samples, per-step entropy cliff, xi=1). 24 tasks x 50 ep."
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/baseline_aac_original_N${N_SAMPLES}_xi${XI}"
