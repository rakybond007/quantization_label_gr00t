#!/bin/bash
#SBATCH --job-name=eval_robocasa_fair_moe_v2_b_only_HF_60k_chkpt_myungkyu_client_robosuite_1p5p1_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_fair_moe_v2_b_only_HF_myungkyu_client.out
#SBATCH --error=out/%A_%a-eval_fair_moe_v2_b_only_HF_myungkyu_client.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="fair_moe_v2_b_only HF baseline re-eval with myungkyu's robocasa_alinvla client env (robosuite 1.5.1 vs ours 1.5.2)."

set -u

PORT=$((9900 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/fair_moe_v2_b_only_as_is_myungkyu_client"
...
PYTHONUNBUFFERED=1 "$SERVER_PY" -u "$BASE_DIR/scripts/inference_service_fair_moe_v2.py" --server \
