#!/bin/bash
#SBATCH --job-name=eval_robocasa_metaq_v1_n8_B_D_lc_0p10_BetaSample_local_60k_myungkyu_client_robosuite_1p5p1_24t_50ep
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_metaq_v1_B_D_Beta_myungkyu_client.out
#SBATCH --error=out/%A_%a-eval_metaq_v1_B_D_Beta_myungkyu_client.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="metaq v1 n8 + B + D gamma=0.10 + Beta-sampled t (local 318767) re-eval with myungkyu's client env."

set -u

PORT=$((9910 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
SERVER_PY="$CONDA_PATH/envs/gr00t/bin/python"
# Use myungkyu's robocasa client env (robosuite 1.5.1 vs our 1.5.2) — testing
# whether framebuffer / robosuite changes affect SR.
CLIENT_PY=/sjw_alinlab2/home/myungkyu/miniconda3/envs/robocasa_alinvla/bin/python

CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_metaq_n8_b_d_lc_0p10/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/metaq_n8_b_d_lc_0p10_as_is_beta_only_myungkyu_client"
...
PYTHONUNBUFFERED=1 "$SERVER_PY" -u "$BASE_DIR/scripts/inference_service_metaq.py" --server \
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 "$CLIENT_PY" -u \
