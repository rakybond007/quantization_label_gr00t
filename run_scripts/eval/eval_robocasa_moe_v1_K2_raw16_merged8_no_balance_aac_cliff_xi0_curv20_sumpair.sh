#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe_v1_K2_raw16_merged8_no_balance_HF_60k_aac_cliff_xi0_curv20_sumpair_moe_selective_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --comment="robocasa MoE K=2 {raw16, merged8} NO_BALANCE + AAC cliff (head=moe_selective N=10, entropy, xi=0 + curvature 2.0, compress_src=main_sumpair trained decoder). Matches baseline+aac best (xi0+curv2.0) for breakwater comparison."

set -u
HF_REPO=prehj/GR00T-N1.5-robocasa-moe-v1-K2-raw16-merged8-b-only-no-metaq-no-balance-60k
PORT=$((9760 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
N_SAMPLES=10
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe_v1_K2_raw16_merged8_b_only_no_metaq_no_balance_aac_cliff_xi0_curv20_sumpair"
