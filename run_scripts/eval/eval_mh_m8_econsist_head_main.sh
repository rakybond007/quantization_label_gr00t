#!/bin/bash
#SBATCH --job-name=eval_mh_m8_ec_main
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8+econsist model eval with main head (16-step)"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_ec_main.out
#SBATCH --error=out/%A_%a-eval_mh_m8_ec_main.err
#SBATCH --time=2-00:00:00

# m8+econsist checkpoint, head=main (16-step decoder, original action).

set -u

PORT=$((8840 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_econsist_discfix/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/mh_m8_econsist/main"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | mh_m8_econsist main head | port=$PORT"
