#!/bin/bash
#SBATCH --job-name=eval_libero_mh_m8_econsist_ens_merged8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_mh_m8_econsist_ens_merged8.out
#SBATCH --error=out/%A_%a-eval_libero_mh_m8_econsist_ens_merged8.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="Eval our libero mh_m8_econsist ensemble_fix→8 (60k) on 4 task suites x 10 tasks x 50 ep"
...
OUTPUT_BASE="$BASE_DIR/output/libero/mh_m8_econsist_ensemble_fix_merged8"
...
PORT=$((8280 + SLURM_ARRAY_TASK_ID))
echo "[i] Array $SLURM_ARRAY_TASK_ID | libero mh_m8_econsist ensemble_fix→8 | port=$PORT"
