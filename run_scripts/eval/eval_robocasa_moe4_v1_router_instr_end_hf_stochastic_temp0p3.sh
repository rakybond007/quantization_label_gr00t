#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe4_v1_K4_b_only_no_metaq_router_instr_end_HF_60k_chkpt_stochastic_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_moe4_v1_b_only_no_metaq_hf.out
#SBATCH --error=out/%A_%a-eval_robocasa_moe4_v1_b_only_no_metaq_hf.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa MoE4 v1 (K=4 u16/c8/c4/n8) + B only + no metaq HF ckpt @60k, STOCHASTIC + inference_temp=0.3 (argmax-biased)."
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe4_v1_b_only_no_metaq_router_instr_end_hf_stochastic_temp0p3"
...
PYTHONUNBUFFERED=1 "$CONDA_PATH/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/inference_service_fair_moe.py" --server \
    ...
    --denoising_steps 4 --head moe --discrete-action-dims 6 11 --moe-stochastic --moe-inference-temp 0.3 \
    > "$SERVER_LOG" 2>&1 &
SPID=$!
READY=0
