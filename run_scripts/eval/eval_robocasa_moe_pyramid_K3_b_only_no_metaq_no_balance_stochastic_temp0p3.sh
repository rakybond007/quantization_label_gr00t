#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe_pyramid_K3_raw16_m8_m4_b_only_no_metaq_no_balance_60k_as_is_router_pick_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_moe_pyramid_K3_as_is.out
#SBATCH --error=out/%A_%a-eval_robocasa_moe_pyramid_K3_as_is.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa MoE pyramid K=3 {raw16, merged8(2x), merged4(4x)} b_only no_metaq no_balance @60k, as_is router pick."
...
PORT=$((10060 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe_pyramid_K3_raw16_m8_m4_b_only_no_metaq_no_balance_stochastic_temp0p3"
...
PYTHONUNBUFFERED=1 "$CONDA_PATH/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/inference_service_fair_moe.py" --server \
    ...
    --denoising_steps 4 --head moe --moe-stochastic --moe-inference-temp 0.3 --discrete-action-dims 6 11 \
    > "$SERVER_LOG" 2>&1 &
SPID=$!
READY=0
