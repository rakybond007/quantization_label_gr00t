#!/bin/bash
#SBATCH --job-name=eval_mh_m8_ens
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8 model (no consist) eval with ensemble_fix head + client-side 16->8 merge"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_ens.out
#SBATCH --error=out/%A_%a-eval_mh_m8_ens.err
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | mh_m8 ensemble_fix→merged8 | port=$PORT"
