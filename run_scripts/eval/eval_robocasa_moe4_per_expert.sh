#!/bin/bash
#SBATCH --job-name=eval_moe4_per_expert
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_moe4_per_expert.out
#SBATCH --error=out/%A_%a-eval_moe4_per_expert.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="Eval per_expert MoE 4-expert robocasa: head=moe + variable-horizon client"

# 24 tasks split into 8 array jobs × 3 tasks each.
# Server: per_expert MoE ckpt with --head=moe (router picks main/m8/m4/n8 per chunk).
# Client: robocasa_service_moe.py handles variable horizon (16/8/4) with replan
#   policy: H=16 → execute 5; H=8 → execute 3; H=4 → execute 2.

set -u

PORT=$((8810 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_moe4_per_expert_body/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe4_per_expert"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | per_expert MoE eval | port=$PORT"
