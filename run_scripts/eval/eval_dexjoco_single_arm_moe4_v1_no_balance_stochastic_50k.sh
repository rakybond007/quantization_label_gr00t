#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_moe4_v1_no_balance_50k_stochastic_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_stochastic.out
#SBATCH --error=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_stochastic.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo 6-task MoE4 v1 (no balance) STOCHASTIC router pick (head=moe + --moe-stochastic) @50k, 50 ep/task. Sample expert by trained router probs instead of argmax — tests soft-prob distribution behavior."

set -u
TASKS=(hammer_nail click_mouse pick_bucket pinch_tongs fold_glasses water_plant)
TASK="${TASKS[$SLURM_ARRAY_TASK_ID]}"
PORT=$((9840 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
DEXJOCO_REPO="$HOME/multigpu_workspace/external_dependencies/dexjoco"
CKPT="$BASE_DIR/ckpt/dexjoco/groot/groot_n1_5_bs64_single_arm_multitask_moe4_v1_no_balance/checkpoint-50000"
CONFIG="$DEXJOCO_REPO/configs/rand_obj/$TASK.yaml"
OUTPUT_BASE="$BASE_DIR/output/dexjoco/single_arm_multitask_moe4_v1_no_balance_50k_stochastic"
