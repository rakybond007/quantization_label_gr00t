#!/bin/bash
#SBATCH --job-name=eval_libero_dual_agree_t0p10
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_dual_agree_t0p10.out
#SBATCH --error=out/%A_%a-eval_libero_dual_agree_t0p10.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="Eval our libero dual agree thresh=0.10 (60k) on 4 task suites x 10 tasks x 50 ep"
...
OUTPUT_BASE="$BASE_DIR/output/libero/mh_m8_econsist_dual_agree_t0p10"
...
PORT=$((8400 + SLURM_ARRAY_TASK_ID))
echo "[i] Array $SLURM_ARRAY_TASK_ID | libero dual agree thresh=0.10 | port=$PORT"
