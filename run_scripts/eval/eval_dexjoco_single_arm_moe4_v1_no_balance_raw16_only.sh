#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_moe4_v1_no_balance_60k_raw16_only_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_raw16_only.out
#SBATCH --error=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_no_balance_raw16_only.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo 6-task MoE4 v1 (no balance) RAW16-only (--moe-force-pick=0) @60k, 50 ep/task. Tests if raw 16-step expert from MoE training matches baseline."

set -u
TASKS=(hammer_nail click_mouse pick_bucket pinch_tongs fold_glasses water_plant)
TASK="${TASKS[$SLURM_ARRAY_TASK_ID]}"
PORT=$((9620 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/single_arm_multitask_moe4_v1_no_balance_60k_raw16_only"
...
"$CONDA_PATH/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/serve_policy_dexjoco.py" \
    ...
    --head moe --denoising-steps 4 --moe-force-pick 0 \
    > "$SERVER_LOG" 2>&1 &
SPID=$!
READY=0
