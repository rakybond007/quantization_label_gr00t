#!/bin/bash
#SBATCH --job-name=eval_mh_m8_distill_m8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8 model (no consist) eval with m8 head (native 8-step compressed)"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_distill_m8.out
#SBATCH --error=out/%A_%a-eval_mh_m8_distill_m8.err
...
PORT=$((8920 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_econsist_discfix_distill/checkpoint-10000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/mh_m8_distill/m8"
