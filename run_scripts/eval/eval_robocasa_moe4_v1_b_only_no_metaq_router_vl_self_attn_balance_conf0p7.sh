#!/bin/bash
#SBATCH --job-name=eval_robocasa_moe4_v1_b_only_no_metaq_router_vl_self_attn_balance_conf0p7_HF_60k_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_robocasa_vl_self_attn_balance_conf0p7.out
#SBATCH --error=out/%A_%a-eval_robocasa_vl_self_attn_balance_conf0p7.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="robocasa MoE4 v1 K=4 b_only no_metaq balance ON EMA-KL (router=vl_self_attn) + conf0p7 gated stochastic. Router-input ablation."

set -u
HF_REPO=prehj/GR00T-N1.5-robocasa-moe4-v1-K4-b-only-no-metaq-router-vl-self-attn-60k
PORT=$((10040 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/moe4_v1_b_only_no_metaq_router_vl_self_attn_balance_conf0p7"
