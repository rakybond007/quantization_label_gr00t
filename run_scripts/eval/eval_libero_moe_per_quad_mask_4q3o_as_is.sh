#!/bin/bash
#SBATCH --job-name=eval_libero_moe_per_quad_mask_4q3o_main_m8_m4_as_is_60k_chkpt
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_per_quad_mask_as_is.out
#SBATCH --error=out/%A_%a-eval_libero_per_quad_mask_as_is.err
#SBATCH --time=1-00:00:00
#SBATCH --comment="Libero MoE per_quad_mask 4-quad × 3-option (main/m8/m4) — as_is (router self-decides per quad)"

set -u
HEAD=moe; MODE=as_is
PORT=$((10000 + 220 + SLURM_ARRAY_TASK_ID))

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/libero/groot/groot_n1_5_bs64_moe_per_quad_mask_4q3o_main_m8_m4/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/libero/moe_per_quad_mask_4q3o_as_is"
...
echo "[i] libero MoE per_quad_mask as_is (head=$HEAD)"
...
for TASK_NAME in "${TASK_NAMES[@]}"; do
    ...
    "$CONDA_PATH/envs/libero/bin/python" "$BASE_DIR/gr00t/eval/libero/eval_taskwise_gr00t_moe.py" \
        ...
        --args.aac-target-envstep-coverage 5 \
