#!/bin/bash
#SBATCH --job-name=eval_dexjoco_dual_arm_5tasks_moe4_v1_balance_60k_conf0p7_50ep_arr5_trial2_seed1
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-4
#SBATCH --output=out/%A_%a-eval_dexjoco_dual_arm_moe_60k_trial2.out
#SBATCH --error=out/%A_%a-eval_dexjoco_dual_arm_moe_60k_trial2.err
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/dual_arm_multitask_moe4_v1_balance_60k_stochastic_conf0p7_trial2"
...
"$CONDA_PATH/envs/dexjoco/bin/python" -u "$BASE_DIR/scripts/dexjoco_eval_gr00t_sync.py" \
    ...
    --seed 1 --output "$OUTPUT_BASE/$TASK" \
