#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_6tasks_pi05_baseline_30k_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_pi05_baseline_30k.out
#SBATCH --error=out/%A_%a-eval_dexjoco_pi05_baseline_30k.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo single-arm 6-task pi05 baseline @30k, 50 ep/task."
...
CKPT="$OPENPI_DIR/checkpoints/pi05_dexjoco_single_arm_baseline/pi05_dexjoco_single_arm_baseline_h100_2gpu_perGPU32_bs64_60k/30000"
CONFIG="$DEXJOCO_REPO/configs/rand_obj/$TASK.yaml"
OUTPUT_BASE="$GR00T_BASE/output/dexjoco/single_arm_multitask_pi05_baseline_30k"
...
"$OPENPI_VENV/bin/python" -u scripts/serve_policy.py \
    --port $PORT \
    policy:checkpoint \
    --policy.config=pi05_dexjoco_single_arm_baseline \
