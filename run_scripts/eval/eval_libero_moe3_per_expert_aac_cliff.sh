#!/bin/bash
#SBATCH --job-name=eval_libero_moe3_per_expert_main_m8_m4_with_aac_cliff_entropy_xi1_60k_chkpt
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --output=out/%A_%a-eval_libero_moe3_aac_cliff.out
#SBATCH --error=out/%A_%a-eval_libero_moe3_aac_cliff.err
#SBATCH --time=1-00:00:00
#SBATCH --comment="Libero MoE3 per_expert eval + AAC cliff (entropy score, xi=1, head=moe_selective N=10)"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | libero MoE3 $MODE (head=$HEAD N=$N xi=$AAC_XI) | port=$PORT"
