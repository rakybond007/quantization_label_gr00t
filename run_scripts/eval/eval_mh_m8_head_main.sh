#!/bin/bash
#SBATCH --job-name=eval_mh_m8_main
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8 model (no consist) eval with main head (16-step)"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_main.out
#SBATCH --error=out/%A_%a-eval_mh_m8_main.err
#SBATCH --time=2-00:00:00

# m8 (no econsist) checkpoint, head=main (16-step decoder, original action).
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/mh_m8/main"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | mh_m8 main head | port=$PORT"
