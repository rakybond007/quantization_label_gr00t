#!/bin/bash
#SBATCH --job-name=eval_robocasa_baseline_aac_cliff_entropy_xi0_curv20_main_n_main_sumpair_60k_24t_50ep_arr8
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --comment="Robocasa BASELINE (non-MoE) + AAC cliff (entropy, xi=0 + curvature-weight 2.0 (entropy+geometric). 24 tasks x 50 ep."
...
PORT=$((8900 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_baseline/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/baseline_aac_cliff_xi0_curv20"
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 "$CONDA_PATH/envs/robocasa_gr00t/bin/python" -u \
        ...
        --aac-xi "$AAC_XI" \
        --aac-compress-src main_sumpair --curvature-weight 2.0 \
