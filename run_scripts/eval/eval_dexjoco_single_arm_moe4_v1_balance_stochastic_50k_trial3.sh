#!/bin/bash
#SBATCH --job-name=eval_dexjoco_single_arm_multitask_moe4_v1_balance_50k_stochastic_trial3_50ep_arr6
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-5
#SBATCH --output=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_balance_stochastic_50k_trial3.out
#SBATCH --error=out/%A_%a-eval_dexjoco_single_arm_moe4_v1_balance_stochastic_50k_trial3.err
#SBATCH --time=4:00:00
#SBATCH --comment="DexJoCo 6-task MoE4 v1 (balance ON) STOCHASTIC @50k trial3 (seed=196). Repeat run to check stability of balance-ON +7~8pp improvement over no_balance."

set -u
TASKS=(hammer_nail click_mouse pick_bucket pinch_tongs fold_glasses water_plant)
TASK="${TASKS[$SLURM_ARRAY_TASK_ID]}"
PORT=$((10300 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50
...
OUTPUT_BASE="$BASE_DIR/output/dexjoco/single_arm_multitask_moe4_v1_balance_50k_stochastic_trial3"
