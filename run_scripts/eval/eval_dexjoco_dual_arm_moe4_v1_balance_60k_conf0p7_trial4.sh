#!/bin/bash
#SBATCH --job-name=eval_dexjoco_dual_arm_5tasks_moe4_v1_balance_60k_conf0p7_50ep_arr5_trial4_seed42
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --exclude=worker-node100
#SBATCH --array=0-4
#SBATCH --output=out/%A_%a-eval_dexjoco_dual_arm_moe_60k_trial4.out
#SBATCH --error=out/%A_%a-eval_dexjoco_dual_arm_moe_60k_trial4.err
#SBATCH --comment="DexJoCo dual-arm 5-task MoE4 v1 balance (conf0p7 stochastic) @60k, 50 ep/task."
...
CKPT="$BASE_DIR/ckpt/dexjoco/groot/groot_n1_5_bs64_dual_arm_multitask_moe4_v1_balance/checkpoint-60000"
CONFIG="$DEXJOCO_REPO/configs/rand_obj/$TASK.yaml"
OUTPUT_BASE="$BASE_DIR/output/dexjoco/dual_arm_multitask_moe4_v1_balance_60k_stochastic_conf0p7_trial4"
...
"$CONDA_PATH/envs/dexjoco/bin/python" -u "$BASE_DIR/scripts/dexjoco_eval_gr00t_sync.py" \
    ...
    --seed 42 --output "$OUTPUT_BASE/$TASK" \
