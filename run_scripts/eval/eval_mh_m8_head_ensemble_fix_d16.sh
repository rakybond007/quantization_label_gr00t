#!/bin/bash
#SBATCH --job-name=eval_mh_m8_ens_d16
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8 model (no consist) eval with ensemble_fix head + client-side 16->8 merge"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_ens_d16.out
#SBATCH --error=out/%A_%a-eval_mh_m8_ens_d16.err
#SBATCH --time=2-00:00:00

# m8 (no econsist) checkpoint, head=ensemble_fix (WLS over main+f2+f4) → 16-step,
# then client-side merge to 8-step (matches m8 head's effective horizon).

set -u

PORT=$((88160 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_discfix/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/mh_m8/ensemble_fix_merged8_d16"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | mh_m8 ens→8 d16 | port=$PORT"
