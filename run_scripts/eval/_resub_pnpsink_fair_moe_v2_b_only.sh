#!/bin/bash
#SBATCH --job-name=resub_PnPCounterToSink_fair_moe_v2_b_only_60k_chkpt_single_job_no_array_no_siblings
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --output=out/%j-resub_PnPCounterToSink_fair_moe_v2_b_only.out
#SBATCH --error=out/%j-resub_PnPCounterToSink_fair_moe_v2_b_only.err
#SBATCH --time=06:00:00
#SBATCH --comment="Single-job resub PnPCounterToSink for fair_moe_v2_b_only. Separate job (not array) so slurm schedules independently to avoid co-located GL contention."
...
OUTPUT_BASE="$BASE_DIR/output/robocasa/fair_moe_v2_b_only_as_is"
