#!/bin/bash
#SBATCH --job-name=eval_mh_m8_ec_m8
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --comment="m8+econsist model eval with m8 head (native 8-step compressed)"
#SBATCH --partition=background
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_mh_m8_ec_m8.out
#SBATCH --error=out/%A_%a-eval_mh_m8_ec_m8.err
#SBATCH --time=2-00:00:00

# m8+econsist checkpoint, head=m8 (native 8-step decoder, model-side compression).

set -u

PORT=$((8850 + SLURM_ARRAY_TASK_ID))
N_EPISODES=50

BASE_DIR="$HOME/multigpu_workspace/Isaac-GR00T"
CONDA_PATH="$HOME/miniconda3"
CKPT="$BASE_DIR/ckpt/robocasa/groot/groot_n1_5_bs64_mh_m8_econsist_discfix/checkpoint-60000"
OUTPUT_BASE="$BASE_DIR/output/robocasa/mh_m8_econsist/m8"
...
echo "[i] Array $SLURM_ARRAY_TASK_ID | mh_m8_econsist m8 head | port=$PORT"
