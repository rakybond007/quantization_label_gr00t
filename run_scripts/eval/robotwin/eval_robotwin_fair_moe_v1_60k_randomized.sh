#!/bin/bash
#SBATCH --job-name=eval_robotwin_fair_moe_v1_K4_b_only_abs_clean50_60k_demo_randomized_50t_50ep_arr10
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --exclude=worker-node1000,worker-node1001,worker-node1002
#SBATCH --output=out/%A_%a-eval_robotwin_fair_moe_v1_60k_randomized.out
#SBATCH --error=out/%A_%a-eval_robotwin_fair_moe_v1_60k_randomized.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="RoboTwin2.0 fair_moe v1 K=4 B-only abs no-metaq ckpt-60000 eval, demo_randomized (Hard), 50 task × 50 ep × array=0-9."
...
CKPT="$BASE_DIR/ckpt/robotwin/groot_n1_5_bs64_fair_moe_clean50_b_only_abs/checkpoint-60000"
TASK_CFG=demo_randomized
CKPT_SETTING=fair_moe_v1_60k
PORT=$((5830 + SLURM_ARRAY_TASK_ID))
OUT="$BASE_DIR/output/robotwin/fair_moe_v1_60k_randomized/arr_${SLURM_ARRAY_TASK_ID}"
mkdir -p out "$OUT"
...
# ---- Server (gr00t env, fair_moe inference) ----
...
PYTHONUNBUFFERED=1 "$CONDA/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/inference_service_fair_moe.py" --server \
    ...
    --denoising_steps 4 --head moe --discrete-action-dims 6 13 \
    > "$SERVER_LOG" 2>&1 &
SPID=$!
