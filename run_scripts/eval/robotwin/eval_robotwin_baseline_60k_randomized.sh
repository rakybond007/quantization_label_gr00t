#!/bin/bash
#SBATCH --job-name=eval_robotwin_baseline_clean50_vanilla_60k_demo_randomized_50t_50ep_arr10
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --exclude=worker-node1000,worker-node1001,worker-node1002
#SBATCH --output=out/%A_%a-eval_robotwin_baseline_60k_randomized.out
#SBATCH --error=out/%A_%a-eval_robotwin_baseline_60k_randomized.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="RoboTwin2.0 baseline ckpt-60000 eval, demo_randomized (Hard), 50 task × 50 ep × array=0-9 (5 task per array)."
...
PORT=$((5810 + SLURM_ARRAY_TASK_ID))
OUT="$BASE_DIR/output/robotwin/baseline_60k_randomized/arr_${SLURM_ARRAY_TASK_ID}"
mkdir -p out "$OUT"
