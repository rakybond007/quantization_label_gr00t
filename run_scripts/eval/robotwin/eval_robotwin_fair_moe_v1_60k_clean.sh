#!/bin/bash
#SBATCH --job-name=eval_robotwin_fair_moe_v1_K4_b_only_abs_clean50_60k_demo_clean_50t_50ep_arr10
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --partition=background
#SBATCH --array=0-9
#SBATCH --exclude=worker-node1000,worker-node1001,worker-node1002
#SBATCH --output=out/%A_%a-eval_robotwin_fair_moe_v1_60k_clean.out
#SBATCH --error=out/%A_%a-eval_robotwin_fair_moe_v1_60k_clean.err
#SBATCH --time=2-00:00:00
#SBATCH --comment="RoboTwin2.0 fair_moe v1 K=4 B-only abs no-metaq ckpt-60000 eval, demo_clean (Easy), 50 task × 50 ep × array=0-9."
...
PORT=$((5820 + SLURM_ARRAY_TASK_ID))
OUT="$BASE_DIR/output/robotwin/fair_moe_v1_60k_clean/arr_${SLURM_ARRAY_TASK_ID}"
mkdir -p out "$OUT"
