#!/bin/bash
#SBATCH --job-name=eval_robocasa_gateC_internal_policy_gate_token_k2_quantize_tau0p5_24tasks_50ep
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --array=0-7
#SBATCH --output=out/%A_%a-eval_gateC_internal.out
#SBATCH --error=out/%A_%a-eval_gateC_internal.err
#SBATCH --time=2-00:00:00

# Internal-gate closed-loop eval (plan C): the policy server runs
# head=main_gate (get_action_and_gate) and returns "_gate_prob" with each
# 16-step chunk; the client quantizes the chunk with K2 when p >= TAU, else
# executes raw. No external judge server. Per-decision sidecar:
# $OUTPUT_BASE/<task>/gate_conf.csv (episode,step,conf,quantize,called,instr).
set -u
...
PORT=$((12000 + POFF + SLURM_ARRAY_TASK_ID))
...
OUTPUT_BASE="${OUTPUT_BASE:-$BASE_DIR/output/robocasa/gateC_internal_gated}"
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 PYTHONPATH="$BASE_DIR/scripts:${PYTHONPATH:-}" \
        ...
        --compress-k $K \
        --judge-url internal --judge-threshold $TAU --gate-subchunk 0 --gate-k3-threshold $K3TAU ${COMPENSATE:+--compensate $COMPENSATE} --sysid-json "$SYSID_JSON" \
        >& "$ODIR/eval-$SLURM_ARRAY_TASK_ID.log" &
    MAIN_PIDS+=($!)
done
for pid in "${MAIN_PIDS[@]}"; do wait "$pid"; done
echo "[i] Array $SLURM_ARRAY_TASK_ID done (gateC internal gate, K$K tau=$TAU)."
