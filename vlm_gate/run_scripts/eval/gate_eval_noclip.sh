...
MODE="${MODE:-stage1}"
# **클립을 아예 안 걸리게 한다.** patch_clip_bounds 가 input 과 output 을 같이
# 키워 단위당 변위는 그대로 두고 상한만 올린다. 2.5배에서 최대 병합 명령이
# 3.00 이므로 10 이면 np.clip 이 한 번도 안 물린다. dyn 은 건드리지 않는다.
CLIP_SCALE="${CLIP_SCALE:-10}"
JUDGE="${JUDGE:-cosmos}"
case "$JUDGE" in
  cosmos) JUDGE_SCRIPT=vlm_gate_cosmos.py; JUDGE_MODEL=nvidia/Cosmos3-Nano ;;
  gemma)  JUDGE_SCRIPT=vlm_gate.py;        JUDGE_MODEL=google/gemma-4-12b-it ;;
  *) echo "JUDGE 는 cosmos 또는 gemma"; exit 1 ;;
esac
...
"$COSMOS_VENV/bin/python" -u "$BASE_DIR/scripts/$JUDGE_SCRIPT" --serve \
...
wait_ready "$JUDGE_LOG" "JUDGE READY" "$JPID" "$JUDGE" || exit 1
