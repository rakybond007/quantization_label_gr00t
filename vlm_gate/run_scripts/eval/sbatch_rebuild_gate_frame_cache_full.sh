#!/bin/bash
#SBATCH --job-name=rebuild_gate_frame_cache_robocasa_full_262k_after_wipe_for_gate_module_distillation
#SBATCH --nodes=1
#SBATCH --gpus=1
...
#SBATCH --output=out/%A_%a-rebuild_cache_full.out
#SBATCH --error=out/%A_%a-rebuild_cache_full.err
set -u
: "${MODEL_OUTPUT_DIR:?MODEL_OUTPUT_DIR required}"
...
"$HOME/miniconda3/envs/quant_gate_eval/bin/python" -u scripts/build_gate_frame_cache.py \
  ...
  --out-dir "$MODEL_OUTPUT_DIR" --shard "$SLURM_ARRAY_TASK_ID" --num-shards 10
