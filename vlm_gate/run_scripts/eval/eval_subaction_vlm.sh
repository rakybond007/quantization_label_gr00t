...
#SBATCH --job-name=eval_subaction_vlm_uniform_questions_per_task_candidate_list_scene_picks
#SBATCH --nodes=1
#SBATCH --gpus=2
...
#SBATCH --comment="robocasa: uniform questions for every task; only the candidate list differs (auto from instruction). VLM picks within the list from the scene."
...
OUTPUT_BASE="${OUTPUT_BASE:-$BASE_DIR/output/robocasa/subaction_vlm}"
JPORT=$((20000 + POFF + SLURM_ARRAY_TASK_ID))
CHECKS="${CHECKS:-ratio_checks_sa1}"
...
CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=6 MKL_NUM_THREADS=6 PYTHONUNBUFFERED=1 \
"$COSMOS_VENV/bin/python" -u "$BASE_DIR/scripts/vlm_gate_cosmos.py" \
    --serve --port $JPORT --model "$COSMOS_MODEL" > "$JUDGE_LOG" 2>&1 &
JPID=$!
for _ in $(seq 1 90); do grep -q "JUDGE READY" "$JUDGE_LOG" && break; sleep 10; done
grep -q "JUDGE READY" "$JUDGE_LOG" || { echo "판정기 안 뜸"; tail -20 "$JUDGE_LOG"; exit 1; }
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 PYTHONPATH="$BASE_DIR/scripts:${PYTHONPATH:-}" \
        ...
        --compress-k $K \
        --subaction-vlm --clip-scale $CLIP_SCALE \
        --judge-url "http://localhost:$JPORT" --judge-checks "$CHECKS" \
        --gate-subchunk 16 \
