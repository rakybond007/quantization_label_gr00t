...
#SBATCH --job-name=eval_ratio_gate_ceiling_mode_two_tasks_conf_picks_rate_under_measured_task_ceiling
#SBATCH --nodes=1
#SBATCH --gpus=2
...
OUTPUT_BASE="${OUTPUT_BASE:-$BASE_DIR/output/robocasa/ratio_gate_ceiling}"
...
TASKS_ALL=(OpenDoubleDoor TurnOffMicrowave)
...
for TASK in "${SELECTED[@]}"; do
    ...
    PYTHONUNBUFFERED=1 PYTHONPATH="$BASE_DIR/scripts:${PYTHONPATH:-}" \
        ...
        --conf-quantiles "$BASE_DIR/analysis/ratio_prompt/conf_quantiles.json" \
