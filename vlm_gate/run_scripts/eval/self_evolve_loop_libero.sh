#!/bin/bash
# LIBERO self-evolve loop from the NAIVE seed, judge = gemma|cosmos.
# Each cycle: sbatch the full LIBERO VLM-gated eval (4 suites x 10 tasks x 50ep)
# with the current guidance, wait for the array, then evolve (v2 accept/reject vs
# raw + naive-K2 baselines). Per-(benchmark,model) isolation of guidance/versions/
# log/gating-state so gemma and cosmos runs never collide.
#
# Run on the LOGIN node. Usage: JUDGE_BACKEND=gemma|cosmos self_evolve_loop_libero.sh <cycles> [start]
#   env: N_EPISODES (default 50), FRESH=1 to reseed naive + reset gating.
#        GATE_TTL_MAX>0 enables the TTL skip policy in the eval client and
#        isolates the whole run under TAG libero_<backend>_ttl (its own
#        guidance/versions/log/best-state/output dirs).
set -u
N="${1:-5}"; START="${2:-1}"
BACKEND="${JUDGE_BACKEND:-gemma}"
BASE_DIR="$HOME/quantization_agent_workspace/vlm_gate"
PY="$HOME/miniconda3/bin/python"
SBATCH="$BASE_DIR/run_scripts/eval/eval_libero_gated.sh"
...
for c in $(seq "$START" $((START + N - 1))); do
    OUT="$BASE_DIR/output/libero/${TAG}_cycle${c}"
    echo "[loop-$TAG] ===== cycle $c : eval (4 suites x 10 x ${N_EPISODES}ep, judge=$BACKEND) ====="
    ...
    echo "[loop-$TAG] ===== cycle $c : evolve (v2 vs raw+K2) ====="
    args=(--gate "$OUT/gate" --guidance-file "$GUIDE"
          --root-dir "$BASE_DIR/output/libero" --raw baseline_raw --k2 baseline_K2
