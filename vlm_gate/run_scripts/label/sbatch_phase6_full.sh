#!/bin/bash
#SBATCH --job-name=robocasa_phase6_full_labeling_five_axes_guidance_v5
...
GATE_SYSTEM=aligned $HOME/quantization_agent_workspace/cosmos_judge_venv/bin/python -u scripts/vlm_gate_cosmos.py \
  --serve --port $PORT > output/_gate_distill/p6v5full_judge$S.log 2>&1 &
JP=$!
for i in $(seq 1 120); do sleep 20; grep -q "JUDGE READY" output/_gate_distill/p6v5full_judge$S.log && break; done
grep -q "JUDGE READY" output/_gate_distill/p6v5full_judge$S.log || { tail -20 output/_gate_distill/p6v5full_judge$S.log; exit 1; }
GUIDANCE=phase6  \
