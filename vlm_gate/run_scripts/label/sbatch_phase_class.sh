...
#SBATCH --time=2:00:00
#SBATCH --requeue
#SBATCH --output=out/%j-phcls.out
#SBATCH --error=out/%j-phcls.err
set -u
...
CHECKS=phase_class_checks TAG=phase_class "$RUN_PY" -u scripts/phase_class_label.py "$PORT" || rc=1
N=$(wc -l < analysis/ratio_prompt/phase_class.jsonl 2>/dev/null || echo 0); echo "phase_class: $N 행"; [ "$N" -lt 600 ] && rc=1
exit $rc
