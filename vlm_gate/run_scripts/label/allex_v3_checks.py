...
# 볼 수 있는 것이 앞, 단위 행동은 뒤에 설명으로. 모델은 앞에 오는 절을
# 답한다 -- 볼 수 없는 것(돌리고 있다, 쥠을 유지한다)을 앞에 두면 거기서
# 막히고 뒤의 배치까지 가지 않는다.
#
#     t=6  "각진 상자가 두 손 사이에 끼어 있는가"                 A +2.01
#     t=9  "다른 면이 위로 오게 돌리고 있는가 -- 양쪽에 손을 댄 채"  A  0.00
#     t=10 "두 손이 서로 미는 힘의 균형으로만 붙들려 있는가"        A +0.06
#
# 배치를 앞에 두면 일반성을 잃지 않고도 답이 나온다. 뒤에 붙는 절이 그
# 배치가 어떤 단위 행동인지를 말해주므로 문항은 여전히 태스크를 안 가린다.
_CHECKS = (
 ("A", "Is a hand bearing on each of TWO OPPOSITE SIDES of the same thing, with nothing\n"
       "   under it holding it up -- the grip a robot needs to turn something over?",
  ("it is off what it sat on, held only between the two hands",
   "the two hands have it and an edge is coming free of what it sat on",
   "a hand is on each of two opposite sides, its weight still down",
   "two hands are near it on either side, neither bearing on it",
   "one hand alone is at it, or it rests on what is under it")),
 ("B", "Does the thing under the hand GIVE WAY AND STAY GIVEN -- creasing, folding, a\n"
       "   dent that remains -- the way a bag does and a box does not?",
  ("it is creased and out of shape where the hand has it",
   "its surface is folded in around where the fingers press",
   "the hand is on it and it is beginning to give",
   "such a thing is in the picture and the hands are on something else",
   "what the hands are at keeps its own edges")),
 ("C", "Is the hand OPEN ON IT rather than closed round it -- flat, pressing or shoving,\n"
       "   the way you move something you are not carrying?",
  ("the open flat of the hand is bearing on it",
   "the open hand is on it, not yet bearing",
   "the hand is open and coming down onto it",
   "it is there and the hand at it is closed round it",
   "nothing is under an open hand")),
 ("D", "Are the FINGERS WRAPPED ROUND a firm thing -- closed on it, the way you hold\n"
       "   something you mean to carry?",
  ("the fingers meet round it and it is up off what it sat on",
   "the fingers meet round it and it has not left its place",
   "the fingers are closing round it",
   "it is firm and a hand is at it, still open",
   "what the hand is at gives way, or nothing is in a hand")),
 ("E", "Are BOTH HANDS EMPTY and clear of everything -- the way they are between one\n"
       "   piece of work and the next?",
  ("both are empty with open space all round them",
   "both are empty and drawing back from what was worked",
   "both are empty and reaching toward something, not at it",
   "one is empty and the other is at something",
   "something is held or borne on")),
)
