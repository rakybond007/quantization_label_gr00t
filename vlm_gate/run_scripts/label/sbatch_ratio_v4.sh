...
#SBATCH --time=2:00:00
#SBATCH --requeue
#SBATCH --output=out/%j-ratio4.out
#SBATCH --error=out/%j-ratio4.err
set -u
...
CHECKS=ratio_checks_v4 TAG=ratio_v4 "$RUN_PY" -u scripts/ratio_scene_label2.py "$PORT" || rc=1
N=$(wc -l < analysis/ratio_prompt/ratio_v4.jsonl 2>/dev/null || echo 0); echo "ratio_v4: $N 행"; [ "$N" -lt 600 ] && rc=1
exit $rc
