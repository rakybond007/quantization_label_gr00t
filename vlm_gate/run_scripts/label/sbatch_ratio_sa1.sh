...
#SBATCH --time=2:00:00
#SBATCH --requeue
#SBATCH --output=out/%j-sa1.out
#SBATCH --error=out/%j-sa1.err
set -u
...
CHECKS=ratio_checks_sa1 TAG=ratio_sa1 "$RUN_PY" -u scripts/ratio_scene_label.py "$PORT" || rc=1
N=$(wc -l < analysis/ratio_prompt/ratio_sa1.jsonl 2>/dev/null || echo 0); echo "ratio_sa1: $N 행"; [ "$N" -lt 600 ] && rc=1
exit $rc
