...
#SBATCH --time=2:00:00
#SBATCH --requeue
#SBATCH --output=out/%j-ratio3.out
#SBATCH --error=out/%j-ratio3.err
set -u
...
CHECKS=ratio_checks_v3 TAG=ratio_v3 "$RUN_PY" -u scripts/ratio_scene_label.py "$PORT" || rc=1
N=$(wc -l < analysis/ratio_prompt/ratio_v3.jsonl 2>/dev/null || echo 0); echo "ratio_v3: $N 행"; [ "$N" -lt 600 ] && rc=1
exit $rc
