#!/bin/bash
#SBATCH --job-name=train_gate_module_A_phase5_soft_continuous_computed_flags_variant_sweep
#SBATCH --nodes=1
#SBATCH --gpus=1
...
V=(A B); TAG=${V[$SLURM_ARRAY_TASK_ID]}
OUT="$WS/assets/modules_A/robocasa_module_A_phase5_soft$TAG"; mkdir -p "$OUT"
# 기존 phase5 학습에서 val_AUC 가 5에폭에 정점(0.674) 찍고 30에폭에 0.554 로 내려갔다.
# 과적합 구간을 돌 이유가 없으므로 10에폭.
"$HOME/miniconda3/envs/quant_gate_eval/bin/python" -u scripts/train_gate_module.py \
  ...
  --labels "$WS/assets/labels/robocasa/v6b_phase5_soft$TAG.parquet" \
