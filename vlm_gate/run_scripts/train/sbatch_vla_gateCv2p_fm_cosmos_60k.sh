#!/bin/bash
#SBATCH --job-name=vla_joint_training_groot_n15_robocasa_gateC_v2p_fm_token_pretrained_head_cosmos_60k
#SBATCH --nodes=1
#SBATCH --gpus=2
...
#SBATCH --output=out/%j-vla_gateCv2p_fm_cosmos_60k.out
#SBATCH --error=out/%j-vla_gateCv2p_fm_cosmos_60k.err
...
"$HOME/miniconda3/envs/quant_gate/bin/python" -u "$PRIV/scripts/gr00t_finetune.py" \
    ...
    --run-name gateCv2p_fm_cosmos_60k \
    --batch-size 32 \
    --num-gpus 2 \
    --max-steps 60000 \
    --save-steps 10000 \
