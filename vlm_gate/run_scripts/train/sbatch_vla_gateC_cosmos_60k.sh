#!/bin/bash
#SBATCH --job-name=vla_joint_training_groot_n15_robocasa_quantizability_gate_token_cosmos_labels_60k
#SBATCH --nodes=1
#SBATCH --gpus=2
...
#SBATCH --output=out/%j-vla_gateC_cosmos_60k.out
#SBATCH --error=out/%j-vla_gateC_cosmos_60k.err
...
# logit, cosmos teacher p_yes). Starting weights identical to the baseline
...
"$HOME/miniconda3/envs/quant_gate/bin/python" -u "$PRIV/scripts/gr00t_finetune.py" \
    ...
    --run-name gateC_joint_cosmos_60k \
    --batch-size 32 \
    --num-gpus 2 \
    --max-steps 60000 \
    --save-steps 10000 \
    ...
    --quant-gate-label-tolerance 4 ${EXTRA_ARGS:-}
