#!/bin/bash
#SBATCH --job-name=train_gateA_real_droid_pnp_cosmos_labels_25kstep_gb64_teacher_ablation_vs_luna
...
$HOME/miniconda3/envs/quant_gate_eval/bin/python -u scripts/train_gate_module.py \
  ...
  --labels $CK/labels/real_cosmos_full.parquet \
  --cache-dir "$CACHE" \
  --out-dir "$MODEL_OUTPUT_DIR" \
  --epochs 610 --bs 64 --lr 3e-4 --num-workers 8 \
  --wandb "gateA_real_cosmos_25kstep_gb64" \
