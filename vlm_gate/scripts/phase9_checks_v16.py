"""robocasa v16 후보 = **v10 + 첫 문단의 사실 오류 수정.** 문항은 v10 과 동일하다.

## 무엇이 틀렸나

v10 까지의 첫 문단은 이렇게 말한다:

    how many of its commanded poses could be **dropped**, letting the arm travel
    further between the ones that remain

**`dropped`(버린다)는 사실과 다르다.** ATQ 가 하는 것은 K 개 스텝을 **합쳐서** 한 틱에
실행하는 것이고 그 변위는 그대로 살아 있다. 버리는 것이 아니다. `thinning`(솎아내기)
도 뽑아 버린다는 뜻이라 합친다는 느낌이 없다.

뒷절(`letting the arm travel further between the ones that remain`)이 뜻을 복구하기는
한다 -- 남은 자세들 **사이를 더 멀리 간다**니 변위가 살아 있다는 말이다. 그래서 완전히
틀린 것은 아니고 **앞말이 오해를 부르고 뒷말이 수습하는** 구조다.

그리고 `how much of it` 의 `it` 이 두 가지로 읽힌다 -- 바로 앞이 `one moment` 라서
"이 순간을 얼마나" 로도 읽힌다. allex 는 `the next stretch of its motion` 이라고
명시하는데 robocasa 는 `it` 으로 줄였다.
...
v16  to decide how much of the next stretch of that motion could be run at fewer,
     longer steps -- how many of its commanded poses could be merged into one,
     each remaining step carrying the travel of the ones it absorbed, without
     changing the outcome.
```

2문단의 `What thinning takes away` 도 `What merging takes away` 로 맞춘다.
**`matters most / least` 순위 문장은 건드리지 않는다** -- v11 에서 그것을 걷어냈다가
태스크내 AUC 가 0.815 -> 0.755 로 떨어졌다.

문항 A~F, 척도, 부호, 이미지 장수, 조립 순서는 v10 과 글자까지 같다.

## 주의

GUIDANCE 수정은 지금까지 세 번 다 손해였다(v11 재작성 0.755 · v12 문단 추가 0.741 ·
v15 머리글 제거 0.689, v10 은 0.815). **재고 나서 쓸지 정한다.**
"""
...
GUIDANCE = (
    "You are judging one moment of a kitchen robot's motion, to decide how much of the "
    "next stretch of that motion could be run at fewer, longer steps -- how many of its "
    "commanded poses could be merged into one, each remaining step carrying the travel "
    "of the ones it absorbed, without changing the outcome.\n\n"
