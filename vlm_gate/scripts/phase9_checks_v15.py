"""robocasa v15 후보 = **v10 + 계산 사실에서 머리글 제거.** 문항은 v10 과 동일하다.

allex 계산 사실에만 `MEASURED FROM THE PLANNED MOTION ...` 머리글이 없다. allex 에
넣어 보니 200청크에서 네 문항 전부 같은 것이 40% 뿐이고 conf 순위상관이 +0.771 이라
**allex 를 맞출 수는 없다**(48,086 청크가 그 상태로 확정돼 있다).

그래서 robocasa 를 allex 에 맞춘 판을 만들어 얼마나 달라지는지 잰다. 계산 사실은
`robocasa_v15_descriptors` 를 쓴다. 문항·GUIDANCE·척도·부호는 v10 과 글자까지 같다.
"""
