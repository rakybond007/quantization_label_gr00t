...
OUT=f"{BASE}/output/_gate_distill/openai_motion_struct"
...
tail=("The three images are CONSECUTIVE moments of the SAME episode, ~0.4s apart (t, t+0.4s, t+0.8s), "
      ...
      f"Guidance:\n{G}\n"
      "Before answering, fill these fields from what you SEE (keep each under 8 words):\n"
      "Output ONLY this JSON, in this order:\n"
      "{\"phase\": \"<free-space reach | fine alignment | grasp/release | contact-rich manipulation | retract>\", "
      "\"gripper\": \"<open | closing | closed-holding | opening>\", "
      "\"motion\": \"<what changed across the 3 frames>\", "
      "\"risk\": \"<what could break if actions were merged>\", "
      "\"p_yes\": <number>}")
paths=[]; CH=400
    ...
    with open(p,"w") as fo:
        for e,f,fs in reqs[i:i+CH]:
            ...
            fo.write(json.dumps({"custom_id":f"ep{e:04d}_f{f:03d}","method":"POST","url":"/v1/chat/completions",
                "body":{"model":"gpt-5.6-luna","max_completion_tokens":200,"reasoning_effort":"none",
