...
def judge_views(obs, parity="full"):
    ...
    **정책이 그렇게 학습돼서** 필요한 것이지 판정기가 요구하는 것이 아니다.
    ...
    또 못 가린다 -- 계산 사실 때 그렇게 됐다.
        ...
        unflip  반전만 안 함                 **기본. 실측으로 라벨에 제일 가깝다**
        full    거기에 절반 축소까지         쓰지 말 것 -- 아래
        none    반전한 채로                  옛 동작

    ## 실측 (CloseDrawer 8에피, 오프라인 라벨과의 등급 차이 합)

        none    3.91      A 1.37   B 1.23   E 2.69   신뢰도 폭 0.35
        unflip  1.79      A 2.74   B 1.95   E 2.66   신뢰도 폭 0.35   <- 제일 가깝다
        full    3.04      A 3.00   B 1.02   E 3.00   신뢰도 폭 0.09
        오프라인          A 2.88   B 2.43   E 1.49   신뢰도 폭 0.48

    **`full` 은 판정기를 상수로 만든다.** A 와 E 가 100% 3등급, B 가 99% 1등급
    으로 붙박이고 신뢰도가 0.48~0.57 안에서만 움직인다. 그런 게이트는 무엇도
    못 가른다 -- 역치를 어디에 두든 전부 통과하거나 전부 막힌다.

    라벨 경로에 절반 축소가 있는 것은 맞는데 그대로 재현하면 라벨과 정반대로
    간다(B 2.43 대 1.02, E 1.49 대 3.00). **판정기에 닿을 때의 실제 픽셀 크기가
    두 경로에서 같지 않은 것으로 보인다** -- 서버 쪽 전처리가 한 번 더 걸리는지
    봐야 한다. 안 밝혀진 자리라 여기 적어 둔다.
    """
