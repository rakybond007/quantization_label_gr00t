...
# 그 칸에는 "(짝검정 없음)" 이 붙는다.
#
# **robocasa 사다리는 클립을 푼 조건으로 취급한다.**
#
# 네 칸 중 1.5·2.5 는 `clip_scale 3` 으로 돌았고 1.0·2.0 은 클립이 켜진 채였다.
# 그래도 **한 조건으로 본다** -- robocasa 에서 클립을 풀어도 성공률이 안 움직이는
# 것이 24태스크로 측정돼 있기 때문이다(압축을 건 채로):
#
#     K=3  클립1 0.497 -> 클립3 0.500   +0.002  t +0.16
#     K=4  클립1 0.404 -> 클립4 0.382   -0.022  t -1.52
#
# 배경도 맞는다 -- 병합 명령이 컨트롤러 범위를 벗어나는 비율이 robocasa K2 는
# 7.4% 인데 libero K2 는 40.7% 다. **libero 는 클립이 실제로 무는 벤치이고
# robocasa 는 아니다.** 그래서 libero 사다리는 클립 조건을 섞지 않았다.
LADDERS = {
    "robocasa": [
