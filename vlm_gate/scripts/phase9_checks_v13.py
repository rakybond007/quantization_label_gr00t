"""robocasa 문항 v13 후보 = **v10 + F 문구만.** GUIDANCE 는 v10 그대로다.

v11(GUIDANCE 재작성)과 v12(놓기 문단 추가)는 둘 다 손해였다 -- 국면 순서 재현 여유가
0.088 -> 0.016 · 0.074. **GUIDANCE 수정은 전역으로 작용해 전 문항의 대비를 줄인다.**
그래서 GUIDANCE 는 건드리지 않고 F 문구만 바꾼다.

    v10  F) Is the arm LOWERING what it still holds onto its target right now ...
    v13  F) Is the arm SETTING DOWN what it still holds -- the object in the gripper ...

`LOWERING what it still holds onto its target` 은 `holds onto`(붙잡다)가 한 덩어리로
읽혀 "붙잡고 있는 것을 내린다" 인지 "쥔 것을 목표 위로 내린다" 인지 갈린다. 뒤 조건절이
뜻을 복구하지만 앞머리가 헷갈린다. 덤으로 A 와 F 가 같은 말(`its target`)을 서로 다른
대상에 쓰던 것도 없어진다.

A·B·C·D·E 문항, GUIDANCE, 척도, 부호, 장수, 순서는 v10 과 글자까지 같다. 가중은
잠정이다(R5).
"""
