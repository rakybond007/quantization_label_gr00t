...
v1 의 A 는 72/72 가 3등급으로 붕괴했다(R7 죽은 문항). "멀리 있는가" 는 기준이
없어 회피하기 쉽다. 관찰된 실제 차이 -- OpenDoubleDoor 성공 장면은 문이 화면에
잡히고 실패 장면은 화면 위로 벗어나 있었다 -- 를 그대로 묻는다. 보이냐 안 보이냐는
기준이 필요 없다.

원래 문항 -- 에피소드 하나의 시작 장면이 얼마나 압축을 견딜지 묻는다.
...
묻는다.
    ...
    A OFFSCREEN 닿아야 할 것이 화면 밖이거나 잘려 있는가
...
NAME = {"A": "OFFSCREEN", "B": "CONFINED", "C": "SMALL"}
...
ASK = (
    ...
    "  1 = not true at all\n"
    "A) Is the thing the hand must reach OUT OF VIEW or CUT OFF -- outside this picture,\n"
    "   running off its top or side edge, or hidden behind the arm -- rather than fully\n"
    "   visible with clear space around it?\n"
