...
for _,r in ref.iterrows():
    ...
    if use_act:
        ...
        ax=np.round(A[f:f+16,5:],2).tolist()
...
for i in range(0,len(rows),CH):
    ...
    with open(p,"w") as fo:
        for ep,f,nm,ax in rows[i:i+CH]:
            ...
            if ax is not None:
                txt += ("Planned actions for the next 16 control steps (7 numbers per step: EE delta x,y,z, "
                        "rotation x,y,z, and the GRIPPER command last, 0=open 1=closed):\n"+json.dumps(ax)+"\n")
            txt += ("Answer NO if ANY of these holds over the next 16 steps:\n"
                    "  (1) GRIPPER: the last number changes value (the gripper opens or closes in this window).\n"
                    "  (2) REVERSAL: the sign of any EE delta (x, y or z) flips — the path bends, so averaging "
                    "pairs would cut the corner.\n"
                    "  (3) FINE CONTACT: the gripper is closed (last number 1) AND the EE deltas are small — "
                    "precise placement, insertion or alignment rather than transport.\n"
                    "  (4) DECELERATION: the EE delta magnitudes shrink steadily toward the end of the window.\n"
