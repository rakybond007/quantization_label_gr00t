"""phase9 문항 v2 -- **A·B 두 문항만 다시 겨눈다.** 구조는 건드리지 않는다.
...
돌리기" 를 깎았는데, 그 부류가 압축에 **가장 강한** 부류다.
    ...
    파지 없음(밀기·돌리기) 11개   K2 유지율 1.032   K3 0.933
...
즉 압축이 깨뜨리는 것은 "한 번의 정확한 접촉" 이 아니라 **그리퍼가 닫히는 자리가
얼마나 좁은가** 다. 밀기·돌리기는 닫을 것이 없으니 가장 강하고, 열린 조리대에서
