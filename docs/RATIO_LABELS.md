...
### robocasa 사다리는 클립 조건이 섞여 있다

```
1.0  클립 켬  <- 기준선      1.5  클립 풂
2.0  클립 켬                 2.5  클립 풂
```

그대로 쓴다. **robocasa 에서 클립을 풀어도 성공률이 안 움직이는 것이 측정돼
있다** — 24태스크에서 압축을 건 채로 K=3 이 +0.002(t +0.16), K=4 가 −0.022
(t −1.52) 다. 배경도 맞는다: 병합 명령이 컨트롤러 범위를 벗어나는 비율이
robocasa K2 는 7.4% 인데 libero K2 는 40.7% 다. **libero 는 클립이 실제로 무는
벤치이고 robocasa 는 아니다.** 그래서 libero 사다리는 클립 조건을 섞지 않았다.
