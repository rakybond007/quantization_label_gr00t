...
| `ratio_conf` | 그 배속을 정한 신뢰도 0~1. 확률이 아니라 **순서** 값이다. **접촉 행은 0 이다** |
...
## 온라인 게이트가 라벨과 같은 것을 보게 하려면

평가 도중 VLM 을 부르는 경로(`robocasa_service_compress.py --judge-url`)는
**라벨을 만들 때와 똑같은 것을 보내야 한다.** 다르면 "라벨이 좋은가" 가 아니라
"이 평가용 설정이 좋은가" 를 묻게 된다. 2026-09-10 에 두 자리가 달랐다.

**계산 사실.** 라벨은 `f"{지시문}\n{facts_text(descriptors(a, f))}"` 로 물었다.
온라인은 지시문만 보냈다. 문항 첫 줄이 "The measurements above are stated as
fact" 인데 그 measurements 가 없으면 모델은 다른 질문에 답한다.

**좌우 반전.** 온라인이 `_flipped(obs)` 를 정책과 판정기에 같이 썼다. 반전은
GR00T 가 그렇게 학습돼서 필요한 것이지 판정기가 요구하는 것이 아니다.
**판정기가 거울상 장면을 보고 답하고 있었다.**

같은 태스크(CloseDrawer)로 잰 것. 오프라인 라벨과의 등급 차이 합이다.

    반전한 채로              3.91   A 1.37   B 1.23   E 2.69
    반전 품 (unflip)         1.79   A 2.74   B 1.95   E 2.66   <- 제일 가깝다
    + 절반 축소 (full)       3.04   A 3.00   B 1.02   E 3.00
    오프라인 라벨                   A 2.88   B 2.43   E 1.49

**`full` 은 쓰지 않는다.** 라벨 경로에 절반 축소가 있는 것은 맞는데 그대로
재현하면 판정기가 상수가 된다 -- A·E 가 100% 3등급, B 가 99% 1등급으로 붙박이고
신뢰도가 0.09 폭 안에서만 움직인다. 그런 게이트는 역치를 어디에 두든 전부
통과하거나 전부 막는다. 판정기에 닿을 때의 실제 픽셀 크기가 두 경로에서 같지
않은 것으로 보이고, **아직 안 밝혀진 자리다.**

**E 는 여전히 안 맞는다.** 오프라인 1.49, 온라인 2.66 이고 반전을 풀어도 안
움직였다. A·B·C·D 넷이 맞으므로 실험을 막지는 않되, 게이트가 균일을 못 이길
때 후보로 남는다. 남은 후보는 배치 폭(오프라인 8, 온라인 단건)과 판정기
체크포인트다.

## 접촉 행은 `conf` 도 0 이다 (2026-09-10 부터)

가드가 걸린 행은 `ratio` 를 1.0 으로 박고 **`conf` 도 0 으로 내린다.**

전에는 배속만 박고 신뢰도를 안 건드렸다. 그러면 `conf` 로만 자르는 쪽에서
**가드가 적혀만 있고 작동하지 않는다** -- 접촉 구간의 `conf` 가 0.6 으로 남아
있으면 역치를 넘어 압축된다. 가드 있는 판과 없는 판의 평균 신뢰도가 0.4267 로
똑같았던 것이 그 증상이다. 가드를 걸었는데 신뢰도 분포가 하나도 안 움직였다.

**등급 A~E 는 안 바뀐다.** 비싼 것은 등급이고, 받는 쪽이 규칙을 바꿔 다시
계산할 수 있어야 한다. 어느 행이 가드로 눌린 것인지는 `fixed` 로 안다.

    fixed == 1  ->  ratio 1.0 · conf 0.0 · 등급은 판정기가 답한 그대로

**이것은 라벨 파일의 이야기다.** 평가 도중 VLM 이 그 자리에서 내는 신뢰도는 이
경로를 안 거친다. 온라인 게이트에서 가드가 걸리게 하려면 **액션에서 접촉 경계를
따로 계산해야 한다** -- 라벨을 고쳐도 그쪽은 안 고쳐진다. 두 자리를 헷갈리지 말 것.
