...
LOG_DIR="$BASE_DIR/analysis/_smoke/pyramid_K4_aac_real"; PORT=9789
...
"$CONDA_PATH/envs/gr00t/bin/python" -u "$BASE_DIR/scripts/inference_service_fair_moe.py" --server \
    ...
    --denoising_steps 4 --head moe_selective --n_samples 10 --discrete-action-dims 6 11 \
    > "$LOG_DIR/server.log" 2>&1 &
SPID=$!
for i in $(seq 1 60); do
    if grep -q "Server is ready" "$LOG_DIR/server.log" 2>/dev/null; then break; fi
    if ! kill -0 "$SPID" 2>/dev/null; then echo "K4PYR_AAC_SMOKE FAIL (server died at boot)"; tail -15 "$LOG_DIR/server.log"; exit 1; fi
    sleep 5
done
"$CONDA_PATH/envs/robocasa_gr00t/bin/python" -u "$BASE_DIR/scripts/robocasa_service_selective.py" \
...
if [ $RC -eq 0 ] && grep -q "^episode " "$LOG_DIR/CloseDrawer/prediction.txt" 2>/dev/null && \
   ! grep -qE "AttributeError|Traceback" "$LOG_DIR/server.log"; then
    echo "K4PYR_AAC_SMOKE PASS ($(head -1 $LOG_DIR/CloseDrawer/prediction.txt))"
else
    echo "K4PYR_AAC_SMOKE FAIL (rc=$RC)"
    grep -E "AttributeError|Traceback" "$LOG_DIR/server.log" | head -5
fi
