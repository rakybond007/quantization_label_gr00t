#!/bin/bash
# Smoke: dexjoco MoE no_balance with --moe-force-pick 0 (always raw16 expert).
set -u
TASK=hammer_nail
N_EPISODES=5
PORT=9503
...
OUT="$BASE_DIR/analysis/_smoke/_raw16_only_smoke/$TASK"
mkdir -p "$OUT"
...
echo "[i] Starting server (force_pick=0 raw16) on port $PORT"
...
grep -E "force_pick|head=" "$SERVER_LOG"
