#!/bin/bash
# Smoke: dexjoco MoE no_balance with --moe-force-uniform (random expert pick).
...
OUT="$BASE_DIR/analysis/_smoke/_force_uniform_smoke/$TASK"
mkdir -p "$OUT"
...
echo "[i] Starting server (force_uniform=True) on port $PORT"
