#!/bin/bash
# Smoke: dexjoco MoE no_balance with  (random expert pick).
# Single task (hammer_nail) × 5 episodes. ~5 min on 1 GPU.
set -u
TASK=hammer_nail
N_EPISODES=5
PORT=9601
...
CKPT="$BASE_DIR/ckpt/dexjoco/groot/groot_n1_5_bs64_single_arm_multitask_moe4_v1_balance/checkpoint-30000"
CONFIG="$DEXJOCO_REPO/configs/rand_obj/$TASK.yaml"
OUT="$BASE_DIR/analysis/_smoke/_balance30k_argmax_smoke/$TASK"
mkdir -p "$OUT"
...
echo "[i] Starting server (balance ON 30k argmax) on port $PORT"
...
grep -E "force_uniform|head=" "$SERVER_LOG"
...
echo
echo "SMOKE_BALANCE30K_DONE rc=$RC"
exit $RC
