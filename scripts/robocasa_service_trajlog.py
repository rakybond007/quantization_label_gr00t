...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trajlog_dir", type=str, default=None,
        help="If set, dump per-step state (EEF position + joint positions) to "
             "<trajlog_dir>/traj_ep<NN>.jsonl. Used for trajectory comparison "
             "between two policies on the same seed."
    )
    ...
    if args.server:
        ...
        for i in trange(args.n_episodes):
            # Deterministic per-episode seeding (np + python random + gym reset
            # seed kwarg) covers most RNG sources. Same seed across runs => same
            # post-reset env state.
            ...
            # Log initial post-reset state as step 0 so start marker is the same
            # physical pose for both baseline and MoE runs (same seed -> same init).
