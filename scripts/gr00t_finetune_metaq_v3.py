...
from gr00t.model.gr00t_n1_metaq_v3 import (
    GR00T_N1_5_MetaQV3 as GR00T_N1_5,
    GR00T_N1_5_MetaQV3_Config as GR00T_N1_5_Config,
)
...
class ArgsConfig:
    ...
    # === v3 K=6 extras ===
    use_u8_extra_head: bool = False
    """[v3 K=6] Enable the u8-extra decoder (raw 8-step expert #5)."""

    use_c4_extra_head: bool = False
    """[v3 K=6] Enable the c4-extra decoder (sum-pair from 8, expert #6)."""
    ...
    # ===== Meta-query =====
    n_meta_q: int = 8
    """Number of learnable meta-query tokens appended to the VLM input.  Their
    output features (post eagle_linear) replace vl_embeds.mean() as the router
    input source.  Action decoder cross-attn still uses vl_embeds (router-only)."""

    # ===== [A] Shared timestep across experts (per_expert_h MoE) =====
    moe_shared_t: bool = True
    """[A] Share one timestep across all experts in per_expert_h MoE forward."""

    # ===== [B] EMA-normalize per-expert losses for KL target =====
    moe_normalize_loss_for_kl: bool = True
    """[B] EMA-normalize per-expert losses before KL target softmax (mean-only,
    simpler than z-score).  Removes loss-scale bias across experts."""
    moe_loss_ema_momentum: float = 0.99
    """[B] EMA momentum for per-expert mean-loss tracker."""

    # ===== [C] Balance loss type =====
    moe_balance_type: str = "switch"
    """[C] 'mean_sq' (existing Σ (mean_p - 1/K)^2) or 'switch' (K Σ f_i p_i)."""
...
def main(config: ArgsConfig):
    ...
    if config.use_merged_4_head or config.use_native_8_head or config.use_moe_routing:
        ...
        # v3 K=6 extras: u8_extra + c4_extra decoders
        ah.use_u8_extra_head = bool(config.use_u8_extra_head)
        ah.use_c4_extra_head = bool(config.use_c4_extra_head)
        if ah.use_u8_extra_head and not hasattr(ah, "u8_extra_action_decoder"):
            ah.u8_extra_action_decoder = CategorySpecificMLP(
                num_categories=ah.config.max_num_embodiments,
                input_dim=ah.hidden_size,
                hidden_dim=ah.hidden_size,
                output_dim=ah.action_dim,
            )
            ah.u8_extra_action_decoder.to(device=model.device, dtype=ah.dtype)
        if ah.use_c4_extra_head and not hasattr(ah, "c4_extra_action_decoder"):
            ah.c4_extra_action_decoder = CategorySpecificMLP(
                num_categories=ah.config.max_num_embodiments,
                input_dim=ah.hidden_size,
                hidden_dim=ah.hidden_size,
                output_dim=ah.action_dim,
            )
            ah.c4_extra_action_decoder.to(device=model.device, dtype=ah.dtype)
        ...
        # ===== Meta-query + jimin-dev knobs =====
        ah.n_meta_q = int(config.n_meta_q)
        ...
        if ah.use_moe_routing:
            ...
            if ah.moe_routing_mode == "single_pick":
                assert ah.moe_num_experts in (4, 6), \
                    "v3 fork: --moe-num-experts=4 (u32,c16,u16,c8) or 6 (+u8,c4)"
                assert ah.use_native_8_head, \
                    "v3 fork requires --use-native-8-head (n8 decoder used for c8)"
                if ah.moe_num_experts == 6:
                    assert getattr(ah, "use_u8_extra_head", False) and getattr(ah, "use_c4_extra_head", False), \
                        "--moe-num-experts=6 also requires --use-u8-extra-head and --use-c4-extra-head"
            ...
            if ah.moe_body_mode == "shared_h16" and not hasattr(ah, "moe_experts"):
                ...
                # v3 layout:
                #   K=4 (v3a): [u32, c16, u16, c8] paired with decoders
                #              [action, m8, m4, n8] at horizons [32, 16, 16, 8].
                #   K=6 (v3b): + [u8, c4] paired with [u8_extra, c4_extra]
                #              at horizons [8, 4].
                assert ah.moe_num_experts in (4, 6), \
                    f"v3 requires moe_num_experts in {{4, 6}}, got {ah.moe_num_experts}"
                if ah.moe_num_experts == 6:
                    assert getattr(ah, "use_u8_extra_head", False) and getattr(ah, "use_c4_extra_head", False), \
                        "--moe-num-experts=6 also requires --use-u8-extra-head and --use-c4-extra-head"
                H = ah.config.action_horizon          # 16
                ah.moe_expert_horizons = [2 * H, H, H, H // 2]  # 32, 16, 16, 8
                if ah.moe_num_experts == 6:
                    ah.moe_expert_horizons.append(H // 2)         # u8 (8)
                    ah.moe_expert_horizons.append(H // 4)         # c4 (4)
            if not hasattr(ah, "_moe_forward_step"):
                ah.register_buffer("_moe_forward_step", torch.zeros((), dtype=torch.long), persistent=True)
            # [B] per-expert loss EMA (for KL target scale normalization). The
            # action head's __init__ registers this when use_moe_routing is set
            # at construction; here we backstop in case the action head was
            # built from a base config that didn't have use_moe_routing yet.
        ...
        for attr in [
            ...
            "use_u8_extra_head", "use_c4_extra_head",
