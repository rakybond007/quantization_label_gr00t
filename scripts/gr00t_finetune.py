...
class ArgsConfig:
    ...
    # === Quantizability gate token (plan C joint training) ===
    use_quant_gate: bool = False
    """Attach the one-way quantizability gate token + dedicated readout to the
    action head and add lambda * BCE(gate_logit, teacher p_yes) to the loss."""

    quant_gate_labels: str = ""
    """Parquet with columns episode_index, frame_index, p_yes (teacher labels).
    Frames without a label within tolerance get gate_mask=0 (action loss only)."""

    quant_gate_loss_weight: float = 0.1
    """Lambda for the gate loss (BCE in readout mode, FM-MSE in fm_token mode)."""

    quant_gate_mode: str = "readout"
    """'readout' (C-v1): one-way parallel stream + BCE readout.
    'fm_token' (C-v2): gate is a regular DiT token; its value is generated by
    flow matching jointly with the action chunk (target = [actions; 2p-1])."""

    quant_gate_oneway: bool = False
    """C-v3 (fm_token only): one-way self-attention mask — the gate token may
    attend all tokens, but action/state tokens can never attend the gate."""

    quant_gate_pixel: bool = False
    """C-v4 (fm_token only): small learned conv encoder over 3 downsampled
    (112x112) camera views, added to the gate token's input embedding."""

    quant_gate_pixel_size: int = 112
    """Downsample size for the gate pixel views."""

    quant_gate_label_tolerance: int = 4
    """Max |frame - labeled_frame| to reuse a nearby label (labels are every-8)."""
...
class GateLabeledLeRobotDataset(LeRobotSingleDataset):
    """LeRobotSingleDataset + per-frame quantizability teacher labels.

    Joins (episode_index, frame_index) -> p_yes from a parquet. Labels are
    sampled every 8 frames; the nearest label within +/- tolerance frames is
    used. Frames without a label get gate_mask=0 (gate loss masked out).
    """

    def set_gate_labels(self, labels_path: str, tolerance: int = 4):
        import numpy as np
        import pandas as pd

        df = pd.read_parquet(labels_path, columns=["episode_index", "frame_index", "p_yes"])
        self._gate_tol = int(tolerance)
        self._gate_map = {}
        for ep, g in df.groupby("episode_index"):
            g = g.sort_values("frame_index")
            self._gate_map[int(ep)] = (
                g["frame_index"].to_numpy("int64"),
                g["p_yes"].to_numpy("float32"),
            )
        print(f"[quant_gate] loaded {len(df)} labels over {len(self._gate_map)} episodes "
              f"from {labels_path} (tolerance={self._gate_tol})")

    def __getitem__(self, index: int) -> dict:
        import numpy as np

        trajectory_id, base_index = self.all_steps[index]
        raw = self.get_step_data(trajectory_id, base_index)
        gate_px = None
        if getattr(self, "_gate_pixel", False):
            from gr00t.model.action_head.flow_matching_action_head import (
                downsample_gate_views,
            )
            vkeys = sorted(k for k in raw if k.startswith("video."))
            frames = [np.asarray(raw[k])[-1] for k in vkeys]  # last frame per view
            gate_px = downsample_gate_views(frames, size=getattr(self, "_gate_pixel_size", 112))
        data = self.transforms(raw)
        if gate_px is not None:
            data["gate_pixels"] = gate_px  # (V, S, S, 3) uint8
        p, m = 0.0, 0.0
        ent = getattr(self, "_gate_map", {}).get(int(trajectory_id))
        if ent is not None:
            fr, py = ent
            j = int(np.searchsorted(fr, base_index))
            best, bestd = -1, 1 << 30
            for k in (j - 1, j):
                if 0 <= k < len(fr):
                    d = abs(int(fr[k]) - int(base_index))
                    if d < bestd:
                        bestd, best = d, k
            if best >= 0 and bestd <= self._gate_tol:
                p, m = float(py[best]), 1.0
        data["gate_label"] = np.asarray(p, dtype=np.float32)
        data["gate_mask"] = np.asarray(m, dtype=np.float32)
        return data
...
def main(config: ArgsConfig):
    ...
    if len(config.dataset_path) == 1:
        dataset_cls = GateLabeledLeRobotDataset if config.use_quant_gate else LeRobotSingleDataset
        ...
        if config.use_quant_gate:
            assert config.quant_gate_labels, "--quant-gate-labels is required with --use-quant-gate"
            train_dataset.set_gate_labels(
                config.quant_gate_labels, tolerance=config.quant_gate_label_tolerance
            )
            train_dataset._gate_pixel = bool(config.quant_gate_pixel)
            train_dataset._gate_pixel_size = int(config.quant_gate_pixel_size)
    else:
        assert not config.use_quant_gate, "quant_gate supports single-dataset training only"
        single_datasets = []
    ...
    if config.use_quant_gate:
        ah = model.action_head
        if config.quant_gate_mode == "fm_token":
            ah.attach_quant_gate_fm(loss_weight=config.quant_gate_loss_weight,
                                    oneway=config.quant_gate_oneway,
                                    pixel=config.quant_gate_pixel)
            ah.config.quant_gate_oneway = bool(config.quant_gate_oneway)
            ah.config.quant_gate_pixel = bool(config.quant_gate_pixel)
            model.config.action_head_cfg["quant_gate_oneway"] = bool(config.quant_gate_oneway)
            model.config.action_head_cfg["quant_gate_pixel"] = bool(config.quant_gate_pixel)
        elif not getattr(ah, "use_quant_gate", False):
            ah.attach_quant_gate(loss_weight=config.quant_gate_loss_weight)
        else:
            ah.quant_gate_loss_weight = float(config.quant_gate_loss_weight)
        # Persist so from_pretrained on saved checkpoints rebuilds the gate.
        ah.config.use_quant_gate = True
        ah.config.quant_gate_mode = str(config.quant_gate_mode)
        ah.config.quant_gate_loss_weight = float(config.quant_gate_loss_weight)
        model.config.action_head_cfg["use_quant_gate"] = True
        model.config.action_head_cfg["quant_gate_mode"] = str(config.quant_gate_mode)
        model.config.action_head_cfg["quant_gate_loss_weight"] = float(config.quant_gate_loss_weight)
