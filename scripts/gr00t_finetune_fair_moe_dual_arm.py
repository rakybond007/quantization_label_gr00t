...
from gr00t.model.gr00t_n1_fair_moe import GR00T_N1_5_FairMoe as GR00T_N1_5, GR00T_N1_5_FairMoe_Config as GR00T_N1_5_Config
...
class ArgsConfig:
    ...
    """If set, resize the action head's action_dim after loading pretrained
    weights (used for dual-arm dexjoco where action_dim=44 exceeds the base
    model's 32). Reinit only action-dim-sensitive submodules; preserves the
    pretrained backbone + DiT body."""
    ...
    """Action representation: 'delta' (a_t = s_{t+1} - s_t; compressed targets
    use block-sum on continuous + last-value on discrete dims) or 'abs'
    (a_t = s_{t+1}; compressed targets use block-last-value on ALL dims).
    Use 'abs' for joint-position-control datasets like RoboTwin / Agilex."""
    ...
    moe_pyramid: bool = False  # full-horizon compression pyramid {1x,2x,4x,8x}
    ...
    moe_router_input_type: str = "avg_pool"
    """How to pool vl_embeds into the router input vector.
    'avg_pool' (default, legacy): vl_embeds.mean(dim=1).
    'instruction_end': pick last valid token per sample via vl_attn_mask
      (decoder-only LLM next-token position summarizes image+text via causal attn).
    'qformer_1q': 1-query learnable attention pool (small MultiheadAttention).
    State is always mean-pooled. Output dim = D_vl + D_state in all cases."""

    moe_router_qformer_heads: int = 4
    """Attention heads for the qformer_1q router pool."""
    ...
    moe_shared_t: bool = False
    """[A] Share one timestep across all experts in per_expert_h MoE.  Default
    off to match jimin's emaKL.sh."""
    moe_normalize_loss_for_kl: bool = False
    """[B] EMA-normalize per-expert losses before KL target softmax (jimin
    emaKL trick).  Default off."""
    moe_loss_ema_momentum: float = 0.99
    """[B] EMA momentum for the per-expert mean-loss tracker."""
    moe_balance_type: str = "mean_sq"
    """[C] 'mean_sq' (default, jimin) or 'switch' (K · Σ f_i · p_i)."""

    save_total_limit: int = 6
    """Max number of checkpoints to keep on disk (older ones auto-deleted)."""
...
def main(config: ArgsConfig):
    ...
    if config.use_merged_4_head or config.use_native_8_head or config.use_moe_routing:
        ...
        # Action representation mode (delta vs abs) — controls compression
        # behavior in _compress_actions.
        ah.action_mode = str(config.action_mode)
        if ah.action_mode not in ("delta", "abs"):
            raise ValueError(f"action_mode must be 'delta' or 'abs', got {ah.action_mode!r}")
        print(f"[fair_moe] action_mode = {ah.action_mode!r} "
              f"({'block-last-value all dims' if ah.action_mode == 'abs' else 'block-sum continuous + last-value discrete'})")
        ...
        # Pyramid full-horizon decoders: m4_full (16->4, factor 4) and m2 (16->2,
        # factor 8). Distinct modules from the (first-8) m4/n8 above. Created when
        # --moe-pyramid; K=3 builds m4_full only, K=4 adds m2.
        if bool(getattr(config, "moe_pyramid", False)):
            if not hasattr(ah, "m4_full_action_decoder"):
                ah.m4_full_action_decoder = CategorySpecificMLP(
                    num_categories=ah.config.max_num_embodiments,
                    input_dim=ah.hidden_size, hidden_dim=ah.hidden_size,
                    output_dim=ah.action_dim,
                ).to(device=model.device, dtype=ah.dtype)
            if int(config.moe_num_experts) == 4 and not hasattr(ah, "m2_action_decoder"):
                ah.m2_action_decoder = CategorySpecificMLP(
                    num_categories=ah.config.max_num_embodiments,
                    input_dim=ah.hidden_size, hidden_dim=ah.hidden_size,
                    output_dim=ah.action_dim,
                ).to(device=model.device, dtype=ah.dtype)
        ...
        ah.moe_pyramid = bool(getattr(config, "moe_pyramid", False))
        ...
        # ===== jimin-dev knobs =====
        ah.moe_shared_t = bool(config.moe_shared_t)
        ah.moe_normalize_loss_for_kl = bool(config.moe_normalize_loss_for_kl)
        ah.moe_loss_ema_momentum = float(config.moe_loss_ema_momentum)
        ah.moe_balance_type = str(config.moe_balance_type)
        ah.moe_router_warmup_steps = int(config.moe_router_warmup_steps)
        ah.moe_router_hidden = int(config.moe_router_hidden)
        ah.moe_router_input_type = str(config.moe_router_input_type)
        ah.moe_router_qformer_heads = int(config.moe_router_qformer_heads)
        if ah.moe_router_input_type not in ("avg_pool", "instruction_end", "qformer_1q", "vl_self_attn"):
            raise ValueError(
                f"--moe-router-input-type must be one of "
                f"avg_pool|instruction_end|qformer_1q|vl_self_attn, got {ah.moe_router_input_type!r}"
            )
        ...
        if ah.use_moe_routing:
            assert ah.use_merged_8_head, "MoE requires --use-merged-8-head"
            ...
            # m4 (merged_4) needed except single_pick K=2 ({main,m8}) or pyramid
            # (which uses its own m4_full/m2 decoders, not the first-8 m4).
            if not (ah.moe_routing_mode == "single_pick" and (ah.moe_num_experts == 2 or ah.moe_pyramid)):
                assert ah.use_merged_4_head, \
                    "MoE requires --use-merged-4-head (except --moe-num-experts=2 or --moe-pyramid)"
            if ah.moe_routing_mode == "single_pick":
                assert ah.moe_num_experts in (2, 3, 4), \
                    "Supported: --moe-num-experts=2 (main+m8), 3 (+m4) or 4 (+n8)"
                if ah.moe_num_experts == 4 and not ah.moe_pyramid:
            ...
            if ah.moe_router_input_type == "qformer_1q" and not hasattr(ah, "router_qformer"):
                from gr00t.model.action_head.flow_matching_action_head_fair_moe import (
                    OneQueryAttnPool,
                )
                ah.router_qformer = OneQueryAttnPool(
                    d_model=ah.config.backbone_embedding_dim,
                    n_heads=ah.moe_router_qformer_heads,
                ).to(device=model.device, dtype=ah.dtype)
            if ah.moe_router_input_type == "vl_self_attn" and not hasattr(ah, "router_vl_self_attention"):
                import torch.nn as _nn
                from gr00t.model.action_head.cross_attention_dit import SelfAttentionTransformer
                assert ah.config.vl_self_attention_cfg is not None, \
                    "vl_self_attn router requires vl_self_attention_cfg (use_vlln=True)"
                ah.router_vlln = _nn.LayerNorm(ah.config.backbone_embedding_dim).to(
                    device=model.device, dtype=ah.dtype)
                ah.router_vl_self_attention = SelfAttentionTransformer(
                    **ah.config.vl_self_attention_cfg).to(device=model.device, dtype=ah.dtype)
            if ah.moe_body_mode == "shared_h16" and not hasattr(ah, "moe_experts"):
                ...
                if ah.moe_pyramid:
                    # pyramid {1x,2x,4x,8x}: [16, 8, 4, 2]
                    ah.moe_expert_horizons = [ah.config.action_horizon,
                                              ah.config.action_horizon // 2]
                    if ah.moe_num_experts >= 3:
                        ah.moe_expert_horizons.append(ah.config.action_horizon // 4)  # 4
                    if ah.moe_num_experts == 4:
                        ah.moe_expert_horizons.append(ah.config.action_horizon // 8)  # 2
                else:
                    # v1. K=2: [main_16, m8_8]; K=3: +m4_4; K=4: +n8_8
                    ah.moe_expert_horizons = [
                        ah.config.action_horizon,
                        ah.config.action_horizon // 2,
                    ]
                    if ah.moe_num_experts >= 3:
                        ah.moe_expert_horizons.append(ah.config.action_horizon // 4)
                    if ah.moe_num_experts == 4:
            ...
            # [B] EMA buffer backstop (in case action head was built from a
            # base config without use_moe_routing).
            if not hasattr(ah, "_moe_loss_ema"):
                ah.register_buffer(
                    "_moe_loss_ema",
                    torch.ones(ah.moe_num_experts, dtype=torch.float32),
                    persistent=True,
                )
        ...
        for attr in [
            ...
            "action_mode",
            "use_moe_routing", "moe_body_mode", "moe_num_experts", "moe_pyramid", "moe_expert_n_layers",
            ...
            "moe_shared_t", "moe_normalize_loss_for_kl", "moe_loss_ema_momentum",
            "moe_balance_type",
