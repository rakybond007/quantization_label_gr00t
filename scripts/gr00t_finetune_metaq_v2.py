...
from gr00t.model.gr00t_n1_metaq_v2 import (
    GR00T_N1_5_MetaQV2 as GR00T_N1_5,
    GR00T_N1_5_MetaQV2_Config as GR00T_N1_5_Config,
)
    ...
    if config.use_merged_4_head or config.use_native_8_head or config.use_moe_routing:
        ...
        if ah.use_moe_routing:
            ...
            if ah.moe_body_mode == "shared_h16" and not hasattr(ah, "moe_experts"):
                ...
                # V2 4-decoder set: u16, c8, u24, c12 (non-overlapping output
                # lengths, two disjoint env-step coverage groups).
                assert ah.moe_num_experts == 4, "v2 requires moe_num_experts=4"
                ah.moe_expert_horizons = [
                    ah.config.action_horizon,        # u16 (16)
                    ah.config.action_horizon // 2,   # c8  (8)
                    24,                              # u24 (24)
                    12,                              # c12 (12)
                ]
