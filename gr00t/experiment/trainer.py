...
class DualBrainTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        loss = outputs["loss"]
        # Log auxiliary multi-horizon losses if present
        log_dict = {}
        for k, v in outputs.items():
            if k.startswith("loss_") and isinstance(v, torch.Tensor) and v.numel() == 1:
                log_dict[k] = v.detach().item()
        if log_dict and self.state.global_step % max(1, self.args.logging_steps) == 0:
            self.log(log_dict)
