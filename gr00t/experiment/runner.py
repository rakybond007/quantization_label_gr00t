...
def _maybe_apply_static_graph(wrapped):
    """Call wrapped._set_static_graph() if available (DDP-wrapped model).

    Used together with ddp_find_unused_parameters=True for MoE training where
    some experts may receive zero gradient on a batch (router min_prob clip).
    static_graph=True lets DDP record the param usage pattern in the first
    iteration and reuse it, avoiding the "marked variable ready twice" error
    that find_unused_parameters alone produces.
    """
    if hasattr(wrapped, "_set_static_graph"):
        try:
            wrapped._set_static_graph()
            print("[DDP] static_graph=True applied to wrapped model")
        except Exception as e:
            print(f"[DDP] static_graph not applied: {e}")

from gr00t.data.dataset import LeRobotMixtureDataset, LeRobotSingleDataset
from gr00t.experiment.trainer import DualBrainTrainer
from gr00t.model.gr00t_n1 import GR00T_N1_5
from gr00t.model.transforms import DefaultDataCollator, QwenDataCollator, PaligemmaDataCollator
...
class TrainRunner:
    def __init__(
        ...
        if model.backbone_model_type == "eagle":
            data_collator = DefaultDataCollator()
        elif model.backbone_model_type == "qwen2_5_vl" or model.backbone_model_type == "qwen2_5_vl_7b":
            data_collator = QwenDataCollator(backbone_path=model.backbone_path)
        elif model.backbone_model_type == "paligemma":
            data_collator = PaligemmaDataCollator(backbone_path=model.backbone_path)
    ...
    def create_trainer(
        ...
        # Patch _wrap_model to apply static_graph after DDP wrapping.
        # Only useful when find_unused_parameters=True (e.g. MoE training);
        # harmless otherwise (just no-op).
        if getattr(training_args, "ddp_find_unused_parameters", False):
            _orig_wrap_model = trainer._wrap_model
            def _patched_wrap_model(model_in, training=True, dataloader=None):
                wrapped = _orig_wrap_model(model_in, training=training, dataloader=dataloader)
                _maybe_apply_static_graph(wrapped)
                return wrapped
            trainer._wrap_model = _patched_wrap_model
