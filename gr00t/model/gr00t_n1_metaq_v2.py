...
# alongside the regular VL features) and FlowmatchingActionHeadMetaQV2 (which
...
from .action_head.flow_matching_action_head_metaq_v2 import (
    FlowmatchingActionHeadMetaQV2,
    FlowmatchingActionHeadMetaQV2Config,
)
from .backbone.eagle_backbone_metaq import EagleBackboneMetaQ as _Base
EagleBackboneMetaQ = _Base
...
@dataclass
class GR00T_N1_5_MetaQV2_Config(PretrainedConfig):
    ...
    model_type = "gr00t_n1_5_metaq_v2"
...
class GR00T_N1_5_MetaQV2(PreTrainedModel):
    """GR00T-N1.5 with meta-query routed router input.  Mirrors GR00T_N1_5 but
    instantiates EagleBackboneMetaQ + FlowmatchingActionHeadMetaQV2.  The forward
    contract is identical (returns BatchFeature with loss/action_pred), so
    training and inference scripts can swap GR00T_N1_5 → GR00T_N1_5_MetaQV2
    without other changes."""

    supports_gradient_checkpointing = True
    config_class = GR00T_N1_5_MetaQV2_Config

    def __init__(self, config: GR00T_N1_5_MetaQV2_Config, local_model_path: str = None):
        ...
        if backbone_model_type != "eagle":
            raise NotImplementedError(
                f"GR00T_N1_5_MetaQV2 currently only supports the eagle backbone "
        ...
        action_head_cfg = FlowmatchingActionHeadMetaQV2Config(**ah_cfg_dict)
        self.action_head = FlowmatchingActionHeadMetaQV2(action_head_cfg)
