...
from gr00t.model.gr00t_n1_fair_moe import GR00T_N1_5_FairMoe
...
class Gr00tPolicyFairMoe(BasePolicy):
    """
    ...
    def __init__(
        ...
        """
        Initialize the Gr00tPolicyFairMoe.
    ...
    def _get_action_from_normalized_input(self, normalized_input: Dict[str, Any]):
        ...
        moe_probs = model_pred.get("moe_probs", None)
        ...
        if moe_picked is not None:
            ...
            probs_arr = moe_probs.detach().cpu().float().numpy() if moe_probs is not None else None
            m8_pack = m8_action.float() if m8_action is not None else None
            return (normalized_action, m8_pack, picked_arr, probs_arr)
    ...
    def _get_unnormalized_action(self, normalized_action) -> Dict[str, Any]:
        if isinstance(normalized_action, tuple):
            picked = None
            probs = None
            if len(normalized_action) == 4:
                main_act, m8_act, picked, probs = normalized_action
            elif len(normalized_action) == 3:
            ...
            if probs is not None:
                main_d["_moe_probs"] = np.asarray(probs, dtype=np.float32)
            return main_d
    ...
    def _load_model(self, model_path):
        model = GR00T_N1_5_FairMoe.from_pretrained(model_path, torch_dtype=COMPUTE_DTYPE, backbone_model_type=self.backbone_model_type)
