...
@dataclass
class FlowmatchingActionHeadMetaQV2Config(PretrainedConfig):
...
class FlowmatchingActionHeadMetaQV2(nn.Module):
    config_class = FlowmatchingActionHeadMetaQV2Config
    supports_gradient_checkpointing = True

    def __init__(
        self,
        config: FlowmatchingActionHeadMetaQV2Config,
    ):
        super().__init__()
        ...
        if self.use_moe_routing:
            ...
            if self.moe_body_mode == "shared_h16":
                ...
                self.moe_expert_horizons = [
                    config.action_horizon,             # u16 (16)
                    config.action_horizon // 2,        # c8  (8)
                    24,                                # u24 (24)
                    12,                                # c12 (12)
                ]
                assert self.moe_num_experts == 4, "v2 decoder set requires moe_num_experts=4"
    ...
    def _moe_expert_targets(self, actions, action_mask,
        ...
        """V2: 4-decoder targets with two disjoint env-step coverage groups.

            c=0  u16 = actions[:, :16]                    (env 0..15)
            c=1  c8  = sum-pair(actions[:, :16], 2)       (env 0..15, 8 outputs)
            c=2  u24 = actions_ext[:, :24]                (env 0..23)
            c=3  c12 = sum-pair(actions_ext[:, :24], 2)   (env 0..23, 12 outputs)

        actions_ext (B, >=24, D) and action_mask_ext (B, >=24) come from the
        data config's `action_extended` modality (range(64) loader).  They are
        REQUIRED for v2 — c=2 and c=3 cannot be built from the 16-step
        `actions` alone.
        """
        H = self.config.action_horizon
        H24 = 24
        c1, m1 = actions[:, :H], action_mask[:, :H]                              # u16
        c2, m2 = self._compress_actions(actions[:, :H], action_mask[:, :H], factor=2)  # c8
        if actions_ext is None or action_mask_ext is None:
            raise RuntimeError(
                "v2 action head requires `action_extended` modality.  Use a "
                "MultiHorizon data config (action_indices=range(64)) so the "
                "transform fills `action_input.action_extended`."
            )
        c3, m3 = actions_ext[:, :H24], action_mask_ext[:, :H24]                  # u24
        c4, m4 = self._compress_actions(actions_ext[:, :H24],
                                        action_mask_ext[:, :H24], factor=2)      # c12
