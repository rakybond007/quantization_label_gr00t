...
@dataclass
class FlowmatchingActionHeadFairMoeV3Config(PretrainedConfig):
    ...
    # === v3 extras (K=6 only): u8 + c4 decoders for the 8-step coverage group ===
    use_u8_extra_head: bool = field(default=False,
        metadata={"help": "[v3] Enable the u8-extra decoder (raw 8-step expert #5)."})
    use_c4_extra_head: bool = field(default=False,
        metadata={"help": "[v3] Enable the c4-extra decoder (sum-pair from 8, expert #6)."})
...
class FlowmatchingActionHeadFairMoeV3(nn.Module):
    config_class = FlowmatchingActionHeadFairMoeV3Config
    supports_gradient_checkpointing = True

    def __init__(
        self,
        config: FlowmatchingActionHeadFairMoeV3Config,
    ):
        super().__init__()
        ...
        if self.use_moe_routing:
            ...
            if self.moe_routing_mode == "single_pick":
                ...
                if self.moe_num_experts == 6:
                    assert getattr(self, "use_u8_extra_head", False) and getattr(self, "use_c4_extra_head", False), \
    ...
    def _moe_expert_targets(self, actions, action_mask,
        ...
        H = self.config.action_horizon (16 for robocasa).
        actions_ext (B, >=32, D) + action_mask_ext (B, >=32) come from the
        MultiHorizon data config's `action_extended` modality
        (action_indices=range(64) loader). REQUIRED for v3.
        """
    ...
    def _moe_forward(self, vl_embs, vl_attn_mask, state_features, embodiment_id,
        ...
        v3 layout (see _moe_expert_targets):
          K=4 (v3a): {u32, c16, u16, c8}
          K=6 (v3b): + {u8, c4}

        Decoders are matched 1:1 with experts in order:
          K=4: [action_decoder, m8_action_decoder, m4_action_decoder, n8_action_decoder]
          K=6: + [u8_extra_action_decoder, c4_extra_action_decoder]
        """
        B = actions.shape[0]
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        if self.use_moe_routing:
            ...
            # v3 needs the extended-action sequence to build u32/c16 targets.
            actions_ext = action_input.get("action_extended", None) \
                if hasattr(action_input, "get") else getattr(action_input, "action_extended", None)
            action_mask_ext = action_input.get("action_extended_mask", None) \
                if hasattr(action_input, "get") else getattr(action_input, "action_extended_mask", None)
