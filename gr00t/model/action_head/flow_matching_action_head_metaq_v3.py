...
@dataclass
class FlowmatchingActionHeadMetaQV3Config(PretrainedConfig):
    ...
    # === v3 extras (K=6 only) ===
    use_u8_extra_head: bool = field(default=False,
        metadata={"help": "[v3] Enable u8-extra decoder (raw 8-step expert #5)."})
    use_c4_extra_head: bool = field(default=False,
        metadata={"help": "[v3] Enable c4-extra decoder (sum-pair from 8, expert #6)."})
    ...
    # ===== Meta-Query router input =====
    n_meta_q: int = field(default=8,
        metadata={"help": "Number of learnable meta-query tokens appended to the VLM "
                          "input.  Their output features (post eagle_linear) replace the "
                          "vl_embeds.mean() pool as the router input source.  The action "
                          "decoders still use the original vl_embeds for cross-attn — "
                          "meta_q is router-only."})

    # ===== [A] Shared timestep across experts in per_expert_h =====
    # Each expert in per_expert_h normally samples its own t.  If True, share one
    # t across all experts in a forward step so per-sample loss is comparable
    # across experts and the KL target softmax(-loss/τ) reflects expert quality
    # rather than per-expert t-noise variance.  Noise is NOT shared (would shift
    # body input distribution under sum-pair compression).
    moe_shared_t: bool = field(default=True,
        metadata={"help": "[A] Share timestep across experts in per_expert_h MoE."})

    # ===== [B] EMA loss-scale normalization for KL router target =====
    # Maintain a per-expert running EMA of mean loss; divide losses by it before
    # computing the KL target softmax.  Removes the bias where an expert with
    # intrinsically smaller loss scale (e.g. fewer output positions) is favored.
    # Only the KL target is normalized; the soft-mixture loss is unchanged.
    moe_normalize_loss_for_kl: bool = field(default=True,
        metadata={"help": "[B] EMA-normalize per-expert losses before KL target softmax."})
    moe_loss_ema_momentum: float = field(default=0.99,
        metadata={"help": "[B] EMA momentum for per-expert loss scale (closer to 1 = slower)."})

    # ===== [C] Switch-Transformer balance loss =====
    # 'mean_sq': Σ (mean_p_i - 1/K)^2 (existing)
    # 'switch' : K * Σ f_i * p_i where f_i = argmax-assignment fraction.
    moe_balance_type: str = field(default="switch",
        metadata={"help": "[C] Balance loss type: 'mean_sq' or 'switch'."})
...
class FlowmatchingActionHeadMetaQV3(nn.Module):
    config_class = FlowmatchingActionHeadMetaQV3Config
    supports_gradient_checkpointing = True

    def __init__(
        self,
        config: FlowmatchingActionHeadMetaQV3Config,
    ):
        super().__init__()
        ...
        # v3 K=6 extras (used only when moe_num_experts == 6)
        self.use_u8_extra_head = bool(getattr(config, "use_u8_extra_head", False))
        self.use_c4_extra_head = bool(getattr(config, "use_c4_extra_head", False))
        if self.use_u8_extra_head:
            self.u8_extra_action_decoder = CategorySpecificMLP(
                num_categories=config.max_num_embodiments,
                input_dim=self.hidden_size,
                hidden_dim=self.hidden_size,
                output_dim=self.action_dim,
            )
        if self.use_c4_extra_head:
            self.c4_extra_action_decoder = CategorySpecificMLP(
                num_categories=config.max_num_embodiments,
                input_dim=self.hidden_size,
                hidden_dim=self.hidden_size,
                output_dim=self.action_dim,
        ...
        # ===== Meta-query and jimin-dev derived knobs =====
        self.n_meta_q = int(getattr(config, "n_meta_q", 8))
        self.moe_shared_t = bool(getattr(config, "moe_shared_t", True))
        self.moe_normalize_loss_for_kl = bool(getattr(config, "moe_normalize_loss_for_kl", True))
        self.moe_loss_ema_momentum = float(getattr(config, "moe_loss_ema_momentum", 0.99))
        self.moe_balance_type = str(getattr(config, "moe_balance_type", "switch"))
        ...
        if self.use_moe_routing:
            ...
            if self.moe_routing_mode == "single_pick":
                assert self.moe_num_experts in (4, 6), \
                    "v3 fork: moe_num_experts in {4 (u32,c16,u16,c8), 6 (+u8,c4)}"
                assert self.use_native_8_head, \
                    "v3 fork requires use_native_8_head=True (n8 decoder used for c8)"
                if self.moe_num_experts == 6:
                    assert self.use_u8_extra_head and self.use_c4_extra_head, \
                        "moe_num_experts=6 requires use_u8_extra_head=True and use_c4_extra_head=True"
            ...
            if self.moe_body_mode == "shared_h16":
                ...
                # V2 4-decoder set with non-overlapping output lengths and TWO
                # disjoint env-step coverage groups (16-coverage and 24-coverage):
                #   c=0  u16  (16 raw, env 0..15)
                #   c=1  c8   (8 sum-pair from 16,  env 0..15, 2 env/action)
                #   c=2  u24  (24 raw, env 0..23)
                #   c=3  c12  (12 sum-pair from 24, env 0..23, 2 env/action)
                # Decoder modules are reused by name (CategorySpecificMLP is
                # horizon-agnostic — it's a per-position projection):
                #   action_decoder       → u16
                #   m8_action_decoder    → c8
                #   m4_action_decoder    → u24  (re-purposed)
                #   n8_action_decoder    → c12  (re-purposed)
                # v3 layout:
                #   K=4 (v3a): [u32, c16, u16, c8] paired with
                #              [action, m8, m4, n8] at horizons [32, 16, 16, 8].
                #   K=6 (v3b): + [u8, c4] paired with [u8_extra, c4_extra]
                #              at horizons [8, 4].
                H = config.action_horizon          # 16
                self.moe_expert_horizons = [2 * H, H, H, H // 2]  # 32, 16, 16, 8
                if self.moe_num_experts == 6:
                    self.moe_expert_horizons.append(H // 2)       # u8 (8)
                    self.moe_expert_horizons.append(H // 4)       # c4 (4)
                assert self.moe_num_experts in (4, 6), \
                    f"v3 supports moe_num_experts in {{4, 6}}, got {self.moe_num_experts}"
            else:
    ...
    def _moe_compute_router_inputs(self, vl_embeds, state_features, meta_q_features=None):
        """Build router input from meta-query (or vl_embeds fallback) + state.

        Meta-query path (preferred): backbone returns ``meta_q_features``
        (B, N, D_vl) — N learnable tokens that were jointly attended over by
        the VLM with vision/text. Mean-pool over N → (B, D_vl). Same dim as
        the legacy vl_embeds.mean() path so the router MLP shape is identical.
        State is pooled and concatenated as before.
        """
        if meta_q_features is not None:
            vl_pooled = meta_q_features.mean(dim=1)          # (B, D_vl)
        else:
            # Fallback for any caller that doesn't pass meta_q (e.g. early
            # debugging): legacy mean-pool of vl_embeds.
    ...
    def _moe_expert_targets(self, actions, action_mask,
                            actions_ext=None, action_mask_ext=None):
        """V3 per-expert (clean, mask) supervision targets.

        K=4 layout (v3a, coverage groups {32, 16}):
            0: u32 (raw 32)              -> actions_ext[:, :32]
            1: c16-from-32 (sum-pair)    -> _compress(actions_ext[:, :32], 2)
            2: u16 (raw 16)              -> actions[:, :H]              (H=16)
            3: c8-from-16  (sum-pair)    -> _compress(actions[:, :H], 2)
        K=6 layout (v3b, coverage groups {32, 16, 8}) adds:
            4: u8  (raw 8)               -> actions[:, :H8]             (H8=8)
            5: c4-from-8   (sum-pair)    -> _compress(actions[:, :H8], 2)

        actions_ext (B, >=32, D) is REQUIRED for v3 — built from the
        MultiHorizon data config's `action_extended` modality
        (action_indices=range(64) loader).
        """
        if actions_ext is None or action_mask_ext is None:
            raise RuntimeError(
                "v3 action head requires `action_extended` modality. Use a "
                "MultiHorizon data config (action_indices=range(64)) so the "
                "transform fills `action_input.action_extended`."
            )
        H = self.config.action_horizon
        H8 = H // 2
        H32 = 2 * H
        u32, m_u32 = actions_ext[:, :H32], action_mask_ext[:, :H32]
        c16, m_c16 = self._compress_actions(actions_ext[:, :H32], action_mask_ext[:, :H32], factor=2)
        u16, m_u16 = actions[:, :H], action_mask[:, :H]
        c8,  m_c8  = self._compress_actions(actions[:, :H], action_mask[:, :H], factor=2)
        out = [(u32, m_u32), (c16, m_c16), (u16, m_u16), (c8, m_c8)]
        if self.moe_num_experts == 6:
            u8, m_u8 = actions[:, :H8], action_mask[:, :H8]
            c4, m_c4 = self._compress_actions(actions[:, :H8], action_mask[:, :H8], factor=2)
            out.append((u8, m_u8))
            out.append((c4, m_c4))
        elif self.moe_num_experts != 4:
            raise ValueError(f"v3 supports moe_num_experts in {{4, 6}}, got {self.moe_num_experts}")
        return out

    def _moe_forward(self, vl_embs, vl_attn_mask, state_features, embodiment_id,
                     actions, action_mask, device, meta_q_features=None,
                     actions_ext=None, action_mask_ext=None):
        ...
        """
        B = actions.shape[0]
        targets = self._moe_expert_targets(
            actions, action_mask,
            actions_ext=actions_ext, action_mask_ext=action_mask_ext,
        )  # list of K (clean, mask) — v2 needs action_extended for u24/c12
        K = self.moe_num_experts
        ...
        if self.moe_body_mode == "per_expert_h":
            ...
            if K == 6:
                decoders.append(self.u8_extra_action_decoder)
                decoders.append(self.c4_extra_action_decoder)
            # [A] Share timestep across experts so per-sample loss is comparable
            # across experts (KL target then reflects expert quality, not t-noise
            # variance).  Noise is NOT shared — each expert samples its own
            # N(0, I) so body input distribution matches the standard one.
        ...
        # --------- router ---------
        # Meta-query path: router input pulls from `meta_q_features` (jointly
        # attended over by the VLM with vision/text), not from a mean of all
        # vl tokens.  State pool concat'd as before.
        router_in = self._moe_compute_router_inputs(
            vl_embs, state_features, meta_q_features=meta_q_features,
        )                                                                      # (B, D)
        ...
        mean_p = router_probs.mean(dim=0)               # (K,)
        ...
        if self.moe_balance_type == "switch":
            ...
            # Stop grad on f_i (it's a discrete count); gradient flows through
            # p_i only — the router is pushed toward putting probability mass
            # where its hard pick lands (encourages decisive, consistent picks).
            with torch.no_grad():
        ...
        # [B] EMA-normalize per-expert losses by their running mean before
        # computing the KL target.  Removes systematic loss-scale bias across
        # experts (e.g. compressed experts have intrinsically smaller MSE).
        # Only the KL teacher signal is normalized; soft-mixture stays raw so
        # the decoder training signal is unchanged.
        # [D] After [B] normalization, add γ · horizon/H_max as length cost in
        # EMA-normalized loss units — sample-conditional push toward shorter
        # experts when quality is similar.
        if self.moe_normalize_loss_for_kl:
            ...
            target_signal = target_basis / ema.unsqueeze(0)               # (B, K), ~order 1
        elif normalize_on:
        ...
        # v3 always has at least K=4. The first 4 expert logs use the same key
        # names (loss_main/m8/m4 + loss_n8) so existing wandb panels work.
        out["loss_n8"]      = losses_b4[:, 3].mean()
        out["router_p_n8"]  = mean_p[3].detach()
        if self.moe_num_experts == 6:
            out["loss_u8_extra"]    = losses_b4[:, 4].mean()
            out["loss_c4_extra"]    = losses_b4[:, 5].mean()
            out["router_p_u8_extra"] = mean_p[4].detach()
            out["router_p_c4_extra"] = mean_p[5].detach()
    ...
    def _moe_forward_per_quad_mask(self, vl_embs, vl_attn_mask, state_features,
        ...
        if hasattr(self, "u8_extra_action_decoder"):
            moe_param_modules.append(self.u8_extra_action_decoder)
        if hasattr(self, "c4_extra_action_decoder"):
            moe_param_modules.append(self.c4_extra_action_decoder)
        anti = sum(
            (p.float().pow(2).sum() for m in moe_param_modules for p in m.parameters()),
            start=torch.zeros((), device=device, dtype=torch.float32),
        )
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        # Meta-query features (B, N, D_vl) — set by EagleBackboneMetaQ; routes
        # exclusively into _moe_compute_router_inputs.  Decoder cross-attn uses
        # vl_embeds (without meta_q) as before.
        meta_q_features = backbone_output.get("meta_q_features", None)
        ...
        # V2: extended action sequence (range(64) loader → action_extended).
        # u24 / c12 decoder targets need actions_ext[:, :24].
        actions_ext = action_input.get("action_extended", None)
        action_mask_ext = action_input.get("action_extended_mask", None)
    ...
    def get_action(self, backbone_output: BatchFeature, action_input: BatchFeature,
        ...
        if head == "moe":
            ...
            if getattr(self, "moe_routing_mode", "single_pick") == "per_quad_mask":
                ...
                router_in = self._moe_compute_router_inputs(
                    vl_embeds, state_features, meta_q_features=meta_q_features,
                )
