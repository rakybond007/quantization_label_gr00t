...
class GR00TTransform(InvertibleModalityTransform):
    ...
    state_horizon: int
    action_horizon: int
    # If > 0, also provide an extended action sequence of this length under
    # the key "action_extended" (used for multi-horizon auxiliary loss).
    # The dataset must be configured to fetch enough future timesteps via
    # action_indices = list(range(extended_action_horizon)).
    extended_action_horizon: int = 0
    ...
    def _apply_vlm_processing(self, batch: dict) -> BatchFeature:
        ...
        elif self.backbone_model_type == "qwen2_5_vl":
            images = batch["images"]  # [V, T, C, H, W]
            images.shape[0]
    ...
    def _prepare_action(self, data: dict):
        ...
        If `extended_action_horizon > 0`, the dataset is providing a longer
        action sequence (e.g. 64 steps). This method returns the FIRST
        `action_horizon` (e.g. 16) for the standard `action`/`action_mask`,
        plus the FULL extended sequence for `action_extended`/mask in
        apply_single below.
        """
        ...
        if self.extended_action_horizon and self.extended_action_horizon > self.action_horizon:
            # Dataset provides a longer sequence; slice the standard prefix.
            assert actions.shape[0] == self.extended_action_horizon, (
                f"Expected dataset to provide {self.extended_action_horizon} steps when "
                f"extended_action_horizon is set, got {actions.shape[0]}"
            )
    ...
    def _prepare_action_extended(self, data: dict):
        """
        Returns the full extended action sequence (length = extended_action_horizon)
        with channel padding to max_action_dim and a per-position mask.

        At episode boundaries the dataset may already pad the trailing positions;
        we preserve whatever it provides and only mask channels (active dims).
        For positions beyond the actual episode length, the mask is set False.
        """
        if "action" not in data or self.extended_action_horizon <= 0:
            return None, None

        actions = data["action"]
        assert actions.shape[0] == self.extended_action_horizon, (
            f"Expected {self.extended_action_horizon} steps, got {actions.shape[0]}"
        )
        n_action_dims = actions.shape[1]
        # Pad channel dimension
        actions = np.pad(actions, ((0, 0), (0, self.max_action_dim - n_action_dims)), "constant")
        mask = np.zeros((self.extended_action_horizon, self.max_action_dim), dtype=bool)
        mask[:, :n_action_dims] = True

        # If the dataset provides a per-position validity mask (e.g. for
        # boundary padding), AND it in. Otherwise assume all positions valid.
        ds_mask = data.get("action_pad_mask") or data.get("action_mask")
        if ds_mask is not None:
            ds_mask = np.asarray(ds_mask)
            if ds_mask.ndim == 1:
                ds_mask = ds_mask[:, None]
            mask = mask & ds_mask
        return actions, mask
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        if self.training:
            ...
            # If extended horizon is enabled, also store the full sequence
            # for multi-horizon auxiliary loss in the action head.
            if self.extended_action_horizon and self.extended_action_horizon > self.action_horizon:
                actions_ext, actions_ext_mask = self._prepare_action_extended(data)
                if actions_ext is not None:
                    transformed_data["action_extended"] = actions_ext
                    transformed_data["action_extended_mask"] = actions_ext_mask
