...
class GR00T_N1_5(PreTrainedModel):
    ...
    def get_action(
        ...
        backbone_outputs = self.backbone(backbone_inputs)
        if head == "main_gate":
            # Quantizability gate token (plan C): identical main denoise loop
            # (bit-identical actions) + gate logit/prob from the one-way token.
            action_head_outputs = self.action_head.get_action_and_gate(
        ...
        if head in ("main", "ensemble", "ensemble_fix", "main_and_m8", "main_gate"):
