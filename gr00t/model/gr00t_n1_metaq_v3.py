...
# alongside the regular VL features) and FlowmatchingActionHeadMetaQV3 (which
...
from .action_head.flow_matching_action_head_metaq_v3 import (
    FlowmatchingActionHeadMetaQV3,
    FlowmatchingActionHeadMetaQV3Config,
)
...
@dataclass
class GR00T_N1_5_MetaQV3_Config(PretrainedConfig):
    ...
    model_type = "gr00t_n1_5_metaq_v3"
...
class GR00T_N1_5_MetaQV3(PreTrainedModel):
    """GR00T-N1.5 with meta-query routed router input.  Mirrors GR00T_N1_5 but
    instantiates EagleBackboneMetaQ + FlowmatchingActionHeadMetaQV3.  The forward
    contract is identical (returns BatchFeature with loss/action_pred), so
    training and inference scripts can swap GR00T_N1_5 → GR00T_N1_5_MetaQV3
    without other changes."""

    supports_gradient_checkpointing = True
    config_class = GR00T_N1_5_MetaQV3_Config

    def __init__(self, config: GR00T_N1_5_MetaQV3_Config, local_model_path: str = None):
        ...
        if backbone_model_type != "eagle":
            raise NotImplementedError(
                f"GR00T_N1_5_MetaQV3 currently only supports the eagle backbone "
        ...
        action_head_cfg = FlowmatchingActionHeadMetaQV3Config(**ah_cfg_dict)
        self.action_head = FlowmatchingActionHeadMetaQV3(action_head_cfg)
