        ...
        for our_modality in ["state", "action"]:
            ...
            for subkey in simplified_modality_meta[our_modality]:
                ...
                for stat_name in le_statistics[le_modality]:
                    ...
                    # Newer LeRobot stats include non-feature-aligned entries
                    # (e.g. scalar "count", which has shape (1,)). These cannot be
                    # sliced by the per-modality index range and are not used for
                    # normalization, so skip any stat that doesn't span the range.
                    if stat.ndim == 0 or stat.shape[0] < state_action_meta.end:
                        continue
