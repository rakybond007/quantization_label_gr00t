...
def get_robocasa_env_fn(
    ...
    def env_fn():
        ...
        if env_name.startswith("robocasa/"):
            ...
            # NEW_EMBODIMENT training/eval expects native robocasa365 keys.
...
def get_gym_env(
    ...
    if env_name.startswith("robocasa") or env_name.startswith("gr1_unified"):
        env_fn = get_robocasa_env_fn(env_name, seed=seed, robocasa_split=robocasa_split, embodiment_tag=embodiment_tag)
    elif env_name.startswith("libero_plus_sim"):
        env_fn = get_libero_plus_env_fn(env_name)
    elif env_name.startswith("libero_pro_sim"):
        env_fn = get_libero_pro_env_fn(env_name)
...
def create_eval_env(
    ...
    print(f"[i] Creating environment {env_name} (env index: {env_idx}) with seed={env_seed}, "
          f"start_episode_id={start_episode_id}")

    env = get_gym_env(env_name, env_idx, total_n_envs, seed=env_seed, robocasa_split=robocasa_split, embodiment_tag=embodiment_tag)
    ...
    while completed_episodes < n_episodes:
                    ...
                    if isinstance(v, np.ndarray):
                        ...
                        for sub_k, sub_v in v.items():
                            if isinstance(sub_v, np.ndarray):
                                print(f"[CLIENT-LOG] Obs[{k}][{sub_k}] shape: {sub_v.shape}, dtype: {sub_v.dtype}")
                            else:
        ...
        for env_idx in range(n_envs):
            ...
            if terminations[env_idx] or truncations[env_idx]:
                ...
                current_successes[env_idx] = False
                is_first_step[env_idx] = True  # Next step in this env will be the first step of a new episode
...
def _load_existing_episode_metadata(
        ...
        existing_successes: List[bool] = [
            episodes[i][2] for i in range(total_episodes)
        ]
...
def run_gr00t_sim_policy(
    ...
    state_delta_indices = np.array(modality_configs["state"].delta_indices) if "state" in modality_configs else None
