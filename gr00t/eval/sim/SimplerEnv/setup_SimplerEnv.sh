...
# Clone SimplerEnv if not already present (this repo doesn't define it as submodule)
if [ ! -d "$SIMPLER_REPO/.git" ]; then
    mkdir -p "$(dirname "$SIMPLER_REPO")"
    git clone --recursive https://github.com/allenzren/SimplerEnv.git "$SIMPLER_REPO"
fi
...
uv pip install "setuptools>=70,<78"  # sapien needs pkg_resources, removed in setuptools>=78
...
uv pip install tianshou==0.5.1 pydantic av zmq torchvision==0.22.0 transformers==4.51.3 pandas
