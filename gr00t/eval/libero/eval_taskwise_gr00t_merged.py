            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    if not action_plan:
                        ...
                        # Query model to get action — apply client-side 16->8
                        # pair-merge (analogous to robocasa_service_merged.py).
                        # Continuous dims sum adjacent pairs; gripper (binary
                        # discrete) uses second-of-pair. Halves the chunk so the
                        # 16-step server output is consumed at half the rate.
                        ...
                        merged_pos  = _pair_merge(action_chunk_dict['action.eef_pos_delta'])
                        merged_rot  = _pair_merge(action_chunk_dict['action.eef_rot_delta'])
                        merged_grip = _pair_merge(action_chunk_dict['action.gripper_close'], discrete=True)
                        action_chunk = np.concatenate([
                            merged_pos, merged_rot, merged_grip.reshape(-1, 1)
                        ], axis=-1)
                        assert (
                            len(action_chunk) >= args.replan_steps
                        ), f"We want to replan every {args.replan_steps} steps, but merged chunk has {len(action_chunk)} steps."
